...
export interface UploadOptions {
  /** Parent folder ref (null = root WattcloudVault/ directory) */
  parentRef?: string;
...
export interface ProviderConfig {
  ...
  /**
   * Server-absolute directory under which the vault lives on the SFTP server.
   * Empty/undefined → vault at `/WattcloudVault` (session root, which is the
   * user's SFTP home on most hosts). When set (e.g. `/wattcloud`) the vault
   * lands at `{sftpBasePath}/WattcloudVault`. Useful for Hetzner Storage Box
   * sub-accounts that are chrooted to a specific subdirectory, or anyone
   * wanting to namespace Wattcloud under a dedicated folder.
   */
  sftpBasePath?: string;
  /**
   ...
   * Empty/undefined keeps the historical behaviour so existing vaults still resolve.
   */
  s3BasePath?: string;
}
...
export interface StorageProvider {
  ...
  /**
   * Canonical storage ref for this vault's `vault_manifest.sc`.
   *
   * Call sites (vault-exists probe, unlock download, conflict-version check)
   * should use this instead of hardcoding the logical `MANIFEST_FILE` path —
   * otherwise providers whose real storage layout differs from that path
   * (notably SFTP, which writes under `{vaultRoot}/data/`) will miss an
   * existing vault and may collide with it on setup.
   */
  manifestRef(): string;

  /**
   * Canonical storage ref for this provider's per-vault SQLite body
   * (`vault_<provider_id>.sc`). Symmetric with `manifestRef()`: SFTP writes
   * bodies under `{vaultRoot}/data/`, while HTTP-style providers use the
   * logical `WattcloudVault/vault_<provider_id>.sc`. Callers that upload
   * and later download a body MUST route through this method so the two
   * sides agree on the path.
   */
  bodyRef(providerId: string): string;

  /**
   * Canonical storage ref for this provider's per-vault journal file
   * (`vault_journal_<provider_id>.j`). Same reasoning as `bodyRef`.
   */
  ...
  /** List contents of a directory. null parentRef = root WattcloudVault/ */
