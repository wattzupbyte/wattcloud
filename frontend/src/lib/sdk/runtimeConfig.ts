 ...
 * URL is accepted. Each of the five provider client IDs must be present
 * and must be a string — an empty string means "provider disabled" (matches
 * .env.example: any BYO_*_CLIENT_ID left blank hides that provider from the
 * SPA). Per-provider enforcement that an ID is actually configured happens
 * at OAuth-initiation time in validateOAuthConfig, not here.
 */
export function validateRuntimeConfig(raw: unknown): WattcloudRuntimeConfig {
  ...
  for (const key of PROVIDER_KEYS) {
    ...
    if (typeof v !== 'string') {
      throw new Error(
        `config.json: clientIds.${key} must be a string (empty = provider disabled)`,
      );
    }
