...
import { acquireRelayCookie, evictRelayCookieCache } from '../relay/RelayAuth';
...
async function doFlush(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      // The relay consumes the stats cookie on each POST /relay/stats, so
      // a cached belief that the cookie is still valid would yield a 401
      // on the next flush. Drop it so the next cycle runs a fresh PoW.
      evictRelayCookieCache('stats');
    }
  } catch {
