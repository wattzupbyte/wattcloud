...
export class ByoUploadStream {
  ...
  private static async uploadLegacy(
    ...
    try {
      ...
      // Provider.uploadStream's onProgress callback receives absolute bytes
      // written, but our public API above is `(pct: number)` (fraction
      // 0..1). Convert before forwarding so the caller's byte-conversion
      // math (pct * file.size) isn't applied to an already-absolute count.
      const providerOnProgress = options?.onProgress
        ? (bytesWritten: number) => {
            options.onProgress!(file.size > 0 ? bytesWritten / file.size : 0);
          }
        : undefined;

      const uploadOptions: UploadOptions | undefined = parentRef
        ? { parentRef, onProgress: providerOnProgress }
        : providerOnProgress
          ? { onProgress: providerOnProgress }
          : undefined;
