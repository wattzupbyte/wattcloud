...
/** Return the per-provider journal file name stored on the cloud provider. */
export function journalFileName(providerId: string): string {
  return `SecureCloud/vault_journal_${providerId}.j`;
}
...
export class VaultJournal {
  ...
  async flush(): Promise<void> {
    ...
    await this.provider.upload(null, journalFileName(this.providerId), out, {});
  }
  ...
  async commit(): Promise<void> {
    ...
    try {
      await this.provider.delete(journalFileName(this.providerId));
    } catch {
  ...
  async replayIfExists(db: import('sql.js').Database): Promise<number> {
    ...
    try {
      const result = await this.provider.download(journalFileName(this.providerId));
    ...
    try {
      ...
      try { await this.provider.delete(journalFileName(this.providerId)); } catch { /* ignore */ }
      this.buffer.length = 0;
      return 0;
    }
