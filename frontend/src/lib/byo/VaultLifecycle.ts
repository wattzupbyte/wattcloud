...
export const MANIFEST_FILE = 'SecureCloud/vault_manifest.sc';
export const VAULT_BODY_PATH_PREFIX = 'SecureCloud/';
...
export async function unlockVault(
  ...
  try {
    ({ data: manifestFileBytes, version: manifestVersion } =
      await provider.download(MANIFEST_FILE));
  } catch (primaryErr) {
  ...
  const vaultSessionId = await byoWorker.Worker.byoVaultOpen(
    params.passphrase,
    header.master_salt,
  ...
  // Find and decrypt the device shard (same as before).
  ...
  const deviceRecord = await getDeviceRecord(vaultIdHex);
  const myDeviceId = deviceRecord?.device_id ?? '';
  ...
  const secondaryFetches = otherProviders.map(async (entry) => {
    try {
      ...
      const { data: secondaryManifestBytes, version: secManifestVersion } =
        await secondaryProvider.download(MANIFEST_FILE);
  ...
  for (const step of unlockPlan.vault_steps) {
    ...
    const vaultFileName = `SecureCloud/vault_${pid}.sc`;
...
async function _doSave(): Promise<void> {
  ...
  try {
    ...
    for (const { provider_id: pid } of savePlan.vault_uploads) {
      ...
      // ref = full path for overwrite semantics (mode=overwrite on Dropbox; idempotent PUT on S3).
      // name = bare filename so providers that auto-prefix 'SecureCloud/' build the right key.
      const bodyRef = `SecureCloud/vault_${pid}.sc`;
    ...
    const { data: currentManifestFile } = await _provider.download(MANIFEST_FILE);
    ...
    for (const pid of savePlan.manifest_upload_targets) {
      ...
      if (!provInst) continue;
      try {
        const { version: uploadedManifestVersion } = await provInst.upload(MANIFEST_FILE, 'vault_manifest.sc', manifestFile, {});
...
export async function removeProvider(providerId: string): Promise<void> {
  ...
  if (provInst) {
    await Promise.allSettled([
      provInst.delete(`SecureCloud/vault_${providerId}.sc`),
      provInst.delete(`SecureCloud/vault_journal_${providerId}.j`),
    ]);
  }
...
function mergeProviderDb(
  ...
  const tables = ['folders', 'files', 'favorites', 'trash', 'share_tokens'] as const;
