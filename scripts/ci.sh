...
#
# Usage:
#   ./scripts/ci.sh                 # full run (lint + test + build + smoke)
#   CI_SKIP_DISK_CHECK=1 ./ci.sh    # skip the preflight disk check
#   CI_SKIP_SMOKE=1 ./ci.sh         # skip the tarball smoke
#   CI_BUILD_DOCKER=1 ./ci.sh       # also build the self-builder Docker image
# =============================================================================
set -euo pipefail
...
# =========================================================================
# 4. Tarball smoke test — run the built binary against /health + /ready.
#    Mirrors what release.yml builds into the tarball artifact. Validates
#    the systemd unit file syntax when systemd-analyze is available.
# =========================================================================
if [[ "${CI_SKIP_SMOKE:-0}" != "1" ]]; then
    step "Packaging — validate systemd unit"
    if command -v systemd-analyze &>/dev/null; then
        # The unit's ExecStart points at /opt/wattcloud/current/bin/byo-relay,
        # which won't exist on the CI host — filter that single warning.
        out=$(systemd-analyze verify "$APP_DIR/packaging/wattcloud.service" 2>&1 \
              | grep -vE 'Command .* is not executable' || true)
        if [ -n "$out" ]; then
            record_fail "wattcloud.service has systemd-analyze issues: $out"
        else
            ok "wattcloud.service syntax OK."
        fi
    else
        warn "systemd-analyze not available — skipping unit syntax check."
    fi

    step "Smoke — tarball-layout /health + /ready + restricted-enrollment flow"
    if bash "$SCRIPT_DIR/byo-smoke.sh" 2>&1; then
        ok "Tarball smoke passed."
    else
        record_fail "Tarball smoke failed."
    fi
fi

# =========================================================================
# 5. Optional: self-builder Docker image (off by default — not a release
#    artifact, kept for users who deliberately want the container path).
# =========================================================================
if [[ "${CI_BUILD_DOCKER:-0}" == "1" ]]; then
    step "Docker — self-builder image (optional)"
    if command -v docker &>/dev/null && docker info &>/dev/null; then
        if (cd "$APP_DIR" && docker build -t wattcloud:ci -f byo-relay/Dockerfile byo-relay) 2>&1; then
            ok "Self-builder image built (tag wattcloud:ci)."
        else
            record_fail "Self-builder image build failed."
        fi
    else
        warn "Docker not available — skipping self-builder image."
    fi
fi

post_run_prune() {
    [[ "${CI_SKIP_PRUNE:-0}" == "1" ]] && return 0
    [[ "${CI_BUILD_DOCKER:-0}" == "1" ]] || return 0
