...
use crate::relay_auth::{
    derive_enrollment_purpose, derive_sftp_purpose, verify_relay_cookie, AppState,
    RELAY_COOKIE_NAME_ENROLL, RELAY_COOKIE_NAME_SFTP,
};
...
pub async fn ws_handler(
    ...
    // Per-category cookie name: the sftp-mode upgrade must present the sftp
    // cookie specifically — the enrollment cookie won't do, and neither will
    // a stats/share cookie from another concurrent flow.
    let mode = query.mode.as_deref().unwrap_or("enrollment");
    let cookie_name = match mode {
        "sftp" => RELAY_COOKIE_NAME_SFTP,
        _ => RELAY_COOKIE_NAME_ENROLL,
    };
    ...
    match mode {
        "sftp" => {
            ...
            if !is_port_allowed(port, &state.config.sftp_allowed_ports) {
                tracing::warn!(port, "sftp relay denied: port not in allowlist",);
                return StatusCode::FORBIDDEN.into_response();
            }
...
mod tests {
    ...
    fn origin_mismatch_rejected() {
        assert!(!is_origin_allowed(
            &hv("https://attacker.example.com"),
            "byo.example.com"
        ));
        assert!(!is_origin_allowed(
            &hv("https://byo.example.com.attacker.com"),
            "byo.example.com"
        ));
    }
