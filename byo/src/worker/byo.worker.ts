...
const CRYPTO_OPS = new Set([
  ...
  'byoEnrollmentSessionGetShard', 'byoEnrollmentClose',
  'oauthBeginFlow',
...
async function handleMessage(request: WorkerRequest): Promise<any> {
  ...
  switch (request.type) {
    ...
    case 'byoDecryptFilename': {
      ...
      const keyResult = decrypt_file_v7(request.encryptedFilenameKeyB64, secKeysJson);
      if (!keyResult || keyResult.error) {
        throw new Error(`Filename key unwrap failed: ${keyResult?.error ?? 'unknown'}`);
      }

      // Step 2: decrypt the filename using the unwrapped symmetric key
      const nameResult = decrypt_filename(request.encryptedFilenameB64, keyResult.data);
