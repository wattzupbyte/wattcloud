...
| Table | Purpose |
|-------|---------|
| `key_versions` | Vault key version history — shared across providers; never deleted. Seeded with a v1 row at vault setup; private keys AES-GCM-wrapped under `HKDF(vault_key, "SecureCloud BYO key_versions wrap v1")` (see SECURITY.md §BYO "Key Versions"). |
...
| `collections` | Photo-album groupings with encrypted name + optional cover file reference, `provider_id` |
| `collection_files` | Junction table — `(collection_id, file_id)` membership, no `provider_id` (inherited from parent collection) |
...
### Optional Primary-Provider Config Transfer

To let a device join a vault from a cold start (without first re-adding the
provider and re-entering credentials locally), the enrollment channel carries
one additional optional message after the shard envelope:

```
Existing Device (A)                                     New Device (B)
                                  ← relay →
        |                                                      |
        | (after SAS confirm) send encrypted_shard             |
        |----------------------------------------------------->|
        |                                                      |
        |                                  (B started cold)    |
        |                              send ready_for_config   |
        |<-----------------------------------------------------|
        | encrypt_payload(primaryConfig_json,                  |
        |                 enc_key, mac_key) =>                 |
        |   payload envelope                                   |
        | send primary_config                                  |
        |----------------------------------------------------->|
...
If B started from the unlock screen with a provider already attached, it
omits `ready_for_config` — A does not send `primary_config` in that case.
If A does not have a config to share (e.g. the flow was reached without an
unlocked vault), A replies with `config_unavailable` so B surfaces a clear
error instead of waiting indefinitely.

### Payload Envelope Wire Format (variable)

```
[nonce (12)] [ct_len_be (4)] [AES-256-GCM(enc_key, plaintext) + 16-byte tag (ct_len)] [HMAC-SHA256(mac_key, nonce||ct_len_be||ct) (32)]
```

Same construction as the shard envelope — encrypt-then-MAC AES-256-GCM +
HMAC-SHA256 under the existing session keys — plus a big-endian `ct_len`
field that is committed into the HMAC. Plaintext is capped at 64 KiB
(`MAX_PAYLOAD_PLAINTEXT_LEN`) so a malicious sender cannot balloon
allocations before the MAC check rejects the frame. The receiver also
re-checks the decoded plaintext length as a belt-and-braces guard against
ciphertext that slipped past static validation.

For SFTP, `primaryConfig` omits `sftpPassword` / `sftpPrivateKey` —
those secrets live inside the source device's WASM heap and are never
persisted, so the receiver is prompted to re-enter them after hydration.
OAuth / S3 / WebDAV configs carry their tokens verbatim.
...
interface ProviderConfig {
  ...
  clientId?: string;        // App client_id (from /config.json at runtime)
  // WebDAV
  serverUrl?: string;
  username?: string;
  password?: string;
...
| Root folder | `WattcloudVault/` (auto-created on first init) |
...
| Root folder | `/drive/root:/WattcloudVault/` |
...
| Root folder | `{serverUrl}/WattcloudVault/` |
| File ref | URL path |
...
| Root folder | `WattcloudVault/` folder (auto-created; folder ID stored in config) |
...
| Root folder | `WattcloudVault/` (auto-created; folder ID stored in config) |
...
| Presigned URL | `GetObject` SigV4 presign (capped at 24 h; internal uses only — share links no longer use presigned URLs) |
...
| Root folder | `/WattcloudVault/` (on remote server) — or `{sftpBasePath}/WattcloudVault/` when configured |
...
The `host_key` handshake frame includes a `relay_version` field. Version history:

| Version | Added |
|---------|-------|
| 1 | Single-shot `write` verb (now retired) |
| 2 | Streaming upload verbs (`write_open` / `write_chunk` / `write_close` / `write_abort`) |
| 3 | Streaming download verbs (`read_open` / `read_chunk` / `read_close`) |

Current client requires `relay_version >= 2` for upload (the v1 single-shot `write` was retired in the v3 bump) and opportunistically uses streaming download when `relay_version >= 3`.

Streaming upload verbs (v2+):

| Verb | Direction | Purpose |
|------|-----------|---------|
| `write_open` | client→relay | Open a new upload session; returns `handle` |
| `write_chunk` | client→relay | Send next binary frame (16 MiB max); relay buffers ciphertext only |
...
Streaming download verbs (v3):

| Verb | Direction | Purpose |
|------|-----------|---------|
| `read_open` | client→relay | Open an SFTP file for streaming reads; returns `handle` |
| `read_chunk` | client→relay | Pull next 256 KiB; two-frame response (JSON + binary, empty binary = EOF) |
| `read_close` | client→relay | Drop the read session and release the SFTP handle |

Relay-side caps: upload buffer holds at most 200 MiB of V7 ciphertext per session; exceeding this limit returns an error and the upload must be aborted and restarted. Concurrent open read sessions capped at 8 per WebSocket connection. Read chunks are 256 KiB each (`READ_CHUNK_SIZE`), matching the client-side progress cadence.
...
The `FooterTrimmer` state machine (`sdk-core/src/crypto/wire_format.rs`) buffers exactly `V7_FOOTER_LEN` bytes, releasing only bytes that are definitively not the footer.
...
#### SFTP Streaming Download (Relay v3)

SFTP doesn't live in the WASM-backed HTTP-provider path — it uses the TypeScript WebSocket shim talking to a `byo-relay` SFTP gateway. Streaming downloads go through the v3 verbs documented in §Relay Protocol v3: `read_open` returns a handle, each `read_chunk` yields a 256 KiB binary frame (empty frame = EOF), `read_close` drops the session. `SftpProvider.downloadStream()` exposes this as a `ReadableStream<Uint8Array>` whose `pull()` drives one `read_chunk` per consumer pull — natural backpressure. On pre-v3 relays the provider falls back to the buffered single-shot `read`.

#### Streaming Share Upload (Client-Side)

Share creation (`ByoDataProvider.createShareLink` and the bundle equivalent) opens `provider.downloadStream(storage_ref)`, peeks the 1709-byte V7 header off the front for fragment-key derivation, then pipes the reconstructed stream straight into `fetch(url, { body: stream, duplex: 'half' })`. The relay's `/relay/share/b2` and `/relay/share/bundle/:id/blob/:bid` routes accept chunked request bodies (`DefaultBodyLimit::disable()`). Browsers without streaming request-body support (pre Chrome 105 / Firefox 105 / Safari 17.4) fall through to a drain-to-buffer path with a console warning; feature detection caches the result per session. Helpers live in `frontend/src/lib/byo/shareUploadStreaming.ts` (`peekHeaderAndResume`, `prependBytes`, `countingTap`, `drainToBuffer`, `supportsRequestStreams`).
...
| `byoEnrollmentSessionEncryptPayload` | `{ sessionId, payloadB64 }` | `{ envelopeB64 }` (variable length) |
| `byoEnrollmentSessionDecryptPayload` | `{ sessionId, envelopeB64 }` | `{ payloadB64 }` (variable length) |
...
Returns the relay's enrollment-gate posture so the SPA can decide which first-run screen to show. Public (no auth). Rate-limited per IP — default 60/min, overridable via `ENROLLMENT_INFO_PER_MIN`.
...
```json
{
  ...
  "version": "<relay semver>"
}
```

- `mode` reflects `WATTCLOUD_ENROLLMENT_MODE`. Unset or unrecognised values fall back to `"open"` so existing installs are unaffected by upgrades.
- `bootstrapped` is `true` iff at least one non-revoked owner device exists in the relay's enrollment store. Used by the SPA to switch between the bootstrap-claim screen and the invite-entry screen (phase 2).
- `version` lets the SPA detect stale-cached-bundle mismatches against a freshly-deployed relay.

#### Restricted-enrollment admin surface (`/relay/admin/*`)

See SECURITY.md §15 for the threat model and data model. Endpoints summary:

| Endpoint | Auth | Rate limit (per IP) | Purpose |
|---|---|---|---|
| `GET /relay/admin/claim/challenge` | public (same limiter as `/relay/auth/challenge`) | 10 / min | PoW challenge for `admin:claim`. Returns `{nonce_id, nonce, difficulty}`. IP-bound; single-use. |
| `GET /relay/admin/redeem/challenge` | public (same limiter) | 10 / min | PoW challenge for `admin:redeem`. |
| `POST /relay/admin/claim` | bootstrap token + PoW answer in body | 5 / 5min + 10 / hour | First owner installs themselves; non-destructive recovery path for subsequent claims. |
| `POST /relay/admin/redeem` | invite code + PoW answer in body | 5 / 5min + 10 / hour | Invitee consumes a single-use invite to enrol as a member. |
| `POST /relay/admin/invite` | owner device cookie | 10 / hour | Mint a new invite code (revealed once in response). |
| `GET /relay/admin/invites` | owner device cookie | — | List active + consumed invites. |
| `DELETE /relay/admin/invites/:id` | owner device cookie | — | Revoke (idempotent delete). |
| `GET /relay/admin/devices` | owner device cookie | — | List all enrolled devices. |
| `DELETE /relay/admin/devices/:id` | owner device cookie | — | Revoke (last-owner guard → 409 `last_owner`). |
| `GET /relay/admin/me` | public | — | Identity probe: returns `{mode, device|null}` so the SPA can pick the right first-run screen in one round-trip. |
| `POST /relay/admin/signout` | device cookie | — | Revoke the current device server-side + clear the cookie. |

Request/response shapes:

- **claim body**: `{token: string (hex/alphanumeric), label: string, pubkey_b64: string (32 raw bytes, base64url no-pad), nonce_id: string, answer: u64}`. The `nonce_id`/`answer` pair is the PoW response to a challenge previously obtained from `GET /relay/admin/claim/challenge`; the server binds nonces to IP + purpose + single-use. Success: `200` + `Set-Cookie: wattcloud_device=<JWT>` + body `{device_id, is_owner: true}`. PoW failure: `403 bad_pow`.
- **redeem body**: `{code: string (11 alphanumerics, optional dashes), label: string, pubkey_b64: string, nonce_id: string, answer: u64}`. Same PoW semantics via `GET /relay/admin/redeem/challenge`. Success: as above with `is_owner: false`.
- **invite body**: `{label: string (≤64 chars), ttl_secs: i64 (clamped [60, 7·86400])}`. Response: `{id, code, label, expires_at}`. The `code` field is **reveal-once** — only its HMAC hash is retained.
- **error shape**: every handler returns the error reason as the response body (e.g. `invalid_invite`, `last_owner`, `rate_limited`, `invalid_token`) so the SPA can map to friendly copy without parsing.

Device cookie (`wattcloud_device`):
- HS256 JWT signed with `RELAY_SIGNING_KEY`.
- Claims: `{sub: device_id, kind: "device", is_owner: bool, iat, exp, jti}`.
- Default TTL 90 days with sliding refresh: middleware mints a fresh cookie when within 7 days of `exp`.
...
One wire format, three source kinds:

- **Single file** — one V7 blob uploaded via `POST /relay/share/b2`; the URL fragment carries the file's `content_key` (optionally Argon2id-wrapped).
- **Folder bundle** — one V7 blob per descendant file + a V7-encrypted manifest JSON listing `{ blob_id, relative_path, content_key }` entries; the URL fragment carries the manifest's `bundle_key`.
- **Collection bundle** — same shape as folder; entries come from the photo-collection's `collection_files`.

Manifest ciphertext is treated like any other blob (V7 header + chunked body + HMAC footer), so the recipient streams + decrypts it with the same pipeline as the content blobs. Per-file `content_key`s never appear on the wire outside the manifest ciphertext.

#### `POST /relay/share/b2` (authenticated, single-file)
...
Response: `{ share_id, expires_at, owner_token }` — `owner_token` is `hex(HMAC-SHA256(share_signing_key, share_id || token_nonce || ":owner"))`. The `token_nonce` is a fresh 16-byte random value the server stores in the share record; a sweeper purge + re-registration therefore invalidates every previously-issued token. `share_signing_key` is loaded from `RELAY_SHARE_SIGNING_KEY` and is distinct from the JWT cookie key (`RELAY_SIGNING_KEY`) — different primitives must not share key material (SC1). The client stores the `owner_token` in vault SQLite; it is the bearer credential for revocation.

#### `POST /relay/share/bundle/init` (authenticated, bundle)

Creates an empty bundle record. Requires a purpose-scoped, single-use relay auth cookie (`purpose = "share:bundle:init"`). Returns `{ share_id, owner_token, bundle_token, expires_at }`. The `bundle_token` is an HMAC-bound capability scoped to this share's upload phase — used by per-blob uploads and the seal call; expires on seal.

#### `POST /relay/share/bundle/:share_id/blob/:blob_id` (bundle-token gated)

Streams one V7 blob into the bundle. Requires `X-Bundle-Token: <token>`. `blob_id` is either a UUID (content blob) or the reserved name `_manifest` (uploaded via seal, not this endpoint). Rejected if the bundle is already sealed.

#### `POST /relay/share/bundle/:share_id/seal` (bundle-token gated)

Uploads the V7-encrypted manifest as `_manifest`, flips the bundle to `sealed=1`, and invalidates the `bundle_token`. Post-seal, recipients can fetch blobs via the unified read API below.

#### `GET /relay/share/:share_id/meta` (unauthenticated, recipient read)

Returns `{ share_id, expires_at, kind, blob_count, total_bytes }` for an active, non-expired, non-revoked, sealed share. Recipients hit this before downloading to learn the share's shape. Returns 404 opaquely for expired, missing, revoked, or unsealed — no distinction.

#### `GET /relay/share/:share_id/blob/:blob_id` (unauthenticated, recipient read)

Streams ciphertext. For single-file shares, `blob_id = "main"`. For bundles, `blob_id` is either `_manifest` or a UUID listed in the manifest. Rate-limited: 10 req/min per share_id, 60 req/min per source IP.

#### `DELETE /relay/share/b2/:share_id` (owner revocation)

Revokes any share (single or bundle). Requires `X-Owner-Token: <token>` header. No relay cookie needed — the token is the bearer credential. The server drops all associated blobs on disk; subsequent GETs 404.

#### `GET /relay/share/headroom` (unauthenticated, operator/UI telemetry)

Returns `{ free_bytes, total_bytes, used_by_shares_bytes, your_remaining_bytes_today, daily_bytes_per_ip }`. No PII, no share IDs. Rate-limited per source IP.

**Share relay security invariants:**
- Server stores V7 ciphertext only. No content keys, no vault keys, no credentials, no filenames, no manifests in plaintext.
- Create endpoints (`POST /relay/share/b2`, `POST /relay/share/bundle/init`) require PoW + purpose-scoped single-use relay auth cookie (signed with `RELAY_SIGNING_KEY`).
- Per-blob and seal calls use a bundle-scoped capability (`X-Bundle-Token`) tied to a single share's upload phase; invalidated on seal.
- Revoke (`DELETE`) uses HMAC ownership tokens keyed on `RELAY_SHARE_SIGNING_KEY` and bound to a server-stored per-record `token_nonce` — tokens cannot be forged from a JWT-key compromise and cannot be replayed across record lifecycles.
- Expiry, revocation, and per-IP daily byte budget are independently enforced server-side and client-side.
...
### Passkey-gated Device Key (optional, per vault)

Users may opt to gate the per-vault device `CryptoKey` — the AES key that wraps the device shard in the manifest header, every `provider_configs` IDB row, WAL entries, and any other at-rest per-device state — behind a WebAuthn authenticator. State lives in a new `device_webauthn` IDB store (`sc-byo` DB v3); row absent = no gate, today's behaviour applies. See SECURITY.md §12 "Passkey-gated device key (BYO)" for full threat model and lifecycle.

Three modes recorded per vault:

- `none` — default; plain `device_crypto_keys` row, `extractable:false`.
- `presence` — `navigator.credentials.get()` required before returning the stored CryptoKey. Behavioural gate only.
- `prf` — device key is random 32 B, wrapped once per enrolled credential under `HKDF(PRF_output, "Wattcloud device key v1")`. The plain `device_crypto_keys` row is deleted; the gate is cryptographically load-bearing.

Changing mode runs a migration in `DeviceKeyMigration.migrateDeviceKey`:

```
1. Download vault_manifest.sc, find this device's slot in the header
2. AES-GCM-decrypt shard with oldKey; AES-GCM-encrypt with newKey + fresh nonce
3. Patch the slot (wrap_iv + encrypted_payload), recompute header HMAC
4. Upload vault_manifest.sc  ← network-touching, runs first for clean rollback
5. For each provider_configs row (this vault): decrypt with oldKey, re-wrap with newKey
6. Clear WAL entries (stale bytes under the new key would fail HMAC on replay anyway)
```

Multi-credential: any number of passkeys can be enrolled for the same vault. Each produces its own PRF output → its own wrapping key → its own copy of the wrapped device key. The same device-key bytes are stashed in WebAuthnGate's session cache during unlock so "Add another passkey" can wrap them under the new credential without an extra decrypt round-trip.

Lockout recovery: the gate never guards vault data itself. If every enrolled passkey is lost, the user re-enrols this device via the recovery key or via QR-link from another device that still has a passkey.

### Passkey Replaces Passphrase (optional, per vault, per device)

A separate toggle (OFF by default) on Settings → Security → Credential Protection lets the user unlock the vault with the passkey alone — the `vault_key` is wrapped under the PRF and stored locally so Argon2id is skipped on unlock. Only available when the gate mode is `prf`; presence mode is a behavioural speed-bump and cannot cryptographically commit to a `vault_key` wrap. See SECURITY.md §12 "Passkey replaces passphrase" for the threat-model trade-off (collapses knowledge factor into possession).

Schema additions (both additive — no DB version bump):

- `WebAuthnCredentialEntry.wrapped_vault_key?: string` — base64 `nonce(12) || ct||tag` covering the 32-byte `vault_key`. Present only when this credential has been extended for passkey-unlock.
- `DeviceWebAuthnRecord.passkey_unlocks_vault?: boolean` — top-level flag; `true` means the unlock path offers a "Unlock with passkey" button driven by these wraps.

Enable flow:

```
1. Confirm modal: explicit disclosure of the security trade-off
2. navigator.credentials.get(allowCredentials = all enrolled PRF creds, prf.eval.first = saved salt)
3. Extract PRF output from the assertion
4. byo_vault_wrap_session_vault_key_with_prf(sessionId, b64(prfOutput)) inside WASM
   → derives HKDF(prfOutput, "Wattcloud vault_key wrap v1"), wraps sess.vault_key
   → returns wrapped bytes as base64
5. Store on the matched credential entry: wrapped_vault_key = base64
6. Set passkey_unlocks_vault = true on the record
```

Only the credential the user touched during step 2 gets a `wrapped_vault_key`. Other enrolled credentials stay without one until the user extends them individually (the Settings UI surfaces the count and a button per credential to touch and extend).

Unlock flow (when `passkey_unlocks_vault === true` and at least one credential has `wrapped_vault_key`):

```
1. ByoUnlock renders a primary "Unlock with passkey" button above the passphrase field
2. navigator.credentials.get(allowCredentials = creds with wrapped_vault_key, prf.eval.first = saved salt)
3. Extract PRF output from the assertion
4. byo_vault_load_session_from_wrapped_vault_key(wrapped_vault_key, b64(prfOutput)) inside WASM
   → derives HKDF, unwraps vault_key, stores it in a fresh VaultSession, returns sessionId
5. Call unlockVault(provider, { preopenedSessionId: sessionId, ... }) — skips Argon2id
6. Rest of unlock (header HMAC verify, shard decrypt, manifest merge) runs unchanged
```

Disable flow: wipe `wrapped_vault_key` on every credential entry and set `passkey_unlocks_vault = false`. No authenticator touch — always safe.

HKDF info `"Wattcloud vault_key wrap v1"` is a frozen protocol constant and lives on CLAUDE.md's append-only HKDF-info list. The wrapping key derives from the PRF output with a distinct info from the device-key wrap, so both ciphertexts are cryptographically independent even though they share a PRF source. Verified in `sdk-core/src/crypto/webauthn.rs::vault_key_wrap_is_domain_separated_from_device_key_wrap`.
...
```jsonc
{
  ...
  "events": [
    ...
    { "kind": "share_resolve", "ts": 1713283250, "share_variant": "B2" },
    { "kind": "share_revoke",  "ts": 1713283260, "share_variant": "B2" },
...
| `STATS_DB_PATH` | `/var/lib/byo-relay/stats.sqlite3` (bare default; systemd unit overrides to `/var/lib/wattcloud/stats.sqlite3` via `StateDirectory`) | SQLite WAL file path |
...
```bash
# On the VPS (byo-relay runs as a systemd service with DynamicUser):
sudo -u $(systemctl show -p User --value wattcloud) \
     /opt/wattcloud/current/bin/byo-admin log --granularity daily
sudo -u $(systemctl show -p User --value wattcloud) \
     /opt/wattcloud/current/bin/byo-admin log --granularity weekly
sudo -u $(systemctl show -p User --value wattcloud) \
     /opt/wattcloud/current/bin/byo-admin log --granularity monthly \
       --from 2026-01-01 --to 2026-04-01
```

Because the service uses `DynamicUser=yes`, the stats DB at
`/var/lib/wattcloud/stats.sqlite3` is owned by a systemd-allocated UID that
does not exist when the service is stopped. Shell into the service's user
namespace via the pattern above (or stop byo-relay and run `byo-admin` as
root — the file's POSIX ACL stays consistent).

### SQLite Schema

Three tables in `/var/lib/wattcloud/stats.sqlite3` (production path via the systemd unit's `StateDirectory=wattcloud`; bare-binary default is `/var/lib/byo-relay/stats.sqlite3`):
