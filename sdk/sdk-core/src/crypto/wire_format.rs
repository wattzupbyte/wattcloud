    ...
    pub fn push(&mut self, data: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        loop {
            ...
            self.chunk_index = self.chunk_index.checked_add(1).ok_or_else(|| {
                CryptoError::InvalidFormat("V7 decrypt: chunk_index overflow (>2^32 chunks)".into())
            })?;
            pos = ct_end;
        }
...
// ─── Share-link encrypt (bundle manifest) ───────────────────────────────────
//
// Bundle shares (folder + collection) need a manifest blob that enumerates
// per-file content_keys and metadata. The manifest is encrypted under a
// fresh `bundle_key` (transported via the URL fragment to the recipient),
// stored on the relay as a normal V7-shaped blob, and decrypted by the
// recipient using `V7ShareDecryptor` — which only needs `file_iv` +
// `key_commitment` from the header. The KEM encapsulation fields are
// vestigial for shares; filling them with zeros keeps the wire format
// stable so the decryptor code path is shared 1:1 with regular V7.
//
// Scope is intentionally narrow: manifests are always small (KB-to-MB),
// buffered whole. Individual file blobs in a bundle are *not* produced by
// this function — they are the existing V7 ciphertext already on the
// sender's provider, uploaded to the relay byte-for-byte.

/// Encrypt a byte slice (typically a JSON manifest) under a known
/// `content_key`, producing a V7-formatted blob the relay + recipient can
/// both consume without special-casing the share path.
///
/// The KEM encapsulation bytes in the header are zeroed; the recipient
/// (`V7ShareDecryptor`) skips them and only verifies `key_commitment ==
/// BLAKE2b-256(content_key || file_iv)`.
pub fn encrypt_manifest_v7(
    manifest: &[u8],
    content_key: &SymmetricKey,
) -> Result<Vec<u8>, CryptoError> {
    let file_iv = generate_nonce()?;

    // Commitment must match what V7ShareDecryptor recomputes on parse.
    let mut kc_input = Vec::with_capacity(44);
    kc_input.extend_from_slice(content_key.as_bytes());
    kc_input.extend_from_slice(file_iv.as_bytes());
    let key_commitment = blake2b_256(&kc_input);

    let mut out = Vec::with_capacity(V7_HEADER_MIN + manifest.len() + 128);

    // Header: byte-identical layout to `V7StreamEncryptor::new`, KEM/EFK
    // bytes zeroed. The layout is a frozen protocol constant; anything that
    // changes the byte offsets will break existing recipients.
    out.push(FILE_FORMAT_V7);
    out.extend_from_slice(file_iv.as_bytes());
    out.extend_from_slice(&[0u8; 32]); // eph_x25519_pub (vestigial for share)
    out.extend_from_slice(&[0u8; 1568]); // mlkem_ct (vestigial for share)
    out.extend_from_slice(&60u32.to_le_bytes()); // efk_len (fixed protocol value)
    out.extend_from_slice(&[0u8; 60]); // efk (vestigial for share)
    out.extend_from_slice(&key_commitment);
    debug_assert_eq!(out.len(), V7_HEADER_MIN);

    // Chunk-encrypt the manifest using the same 512 KiB chunking as regular
    // V7 uploads. A small manifest produces one chunk; a larger one streams
    ...
    // At least one frame must be emitted even for an empty manifest so the
    // body isn't a bare HMAC footer (which would fail size validation).
    loop {
        let end = (offset + CHUNK_SIZE).min(manifest.len());
        let plaintext = &manifest[offset..end];
        let ct = encrypt_file_v7_chunk(plaintext, content_key, &file_iv, chunk_index)?;
        let nonce = v7_chunk_nonce(&file_iv, chunk_index);

        hmac.update(&chunk_index.to_le_bytes());
        hmac.update(&ct);
        ...
        chunk_index = chunk_index.checked_add(1).ok_or_else(|| {
            CryptoError::InvalidFormat("V7 manifest encrypt: chunk_index overflow".into())
        })?;
        offset = end;
    ...
    let footer: [u8; 32] = hmac.finalize().into_bytes().into();
...
impl V7ShareDecryptor {
    ...
    pub fn new(
        ...
        let file_iv = Nonce12::from_slice(header_bytes.get(1..13).ok_or_else(|| {
...
impl V7StreamEncryptor {
    ...
    pub fn push(&mut self, plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        self.chunk_index = self.chunk_index.checked_add(1).ok_or_else(|| {
            CryptoError::InvalidFormat("V7 encrypt: chunk_index overflow (>2^32 chunks)".into())
        })?;
        Ok(frame)
    }
...
mod tests {
    ...
    // ─── Manifest encrypt ↔ V7ShareDecryptor round-trip ─────────────────────

    #[test]
    fn manifest_encrypt_roundtrip_via_share_decryptor() {
        let content_key = SymmetricKey::new([0x5Au8; 32]);
        let manifest = br#"{"version":1,"entries":[{"blob_id":"abc","rel_path":"photo.jpg"}]}"#;
        ...
        dec.finalize(footer).unwrap();
        assert_eq!(plaintext, manifest);
    }

    #[test]
    fn manifest_encrypt_large_crosses_chunk_boundary() {
        let content_key = SymmetricKey::new([0xA5u8; 32]);
        // 700 KiB > 512 KiB chunk → multi-chunk manifest.
        ...
        let plaintext = dec.push(body).unwrap();
        dec.finalize(footer).unwrap();
        assert_eq!(plaintext.len(), manifest.len());
        assert_eq!(plaintext, manifest);
    }

    #[test]
    fn manifest_encrypt_wrong_key_rejected_at_commitment_check() {
        let key_a = SymmetricKey::new([0x11u8; 32]);
        let key_b = SymmetricKey::new([0x22u8; 32]);
        let blob = encrypt_manifest_v7(b"hello", &key_a).unwrap();
        // V7ShareDecryptor isn't Debug, so avoid unwrap_err; explicit match.
        match V7ShareDecryptor::new(&blob[..V7_HEADER_MIN], key_b.as_bytes()) {
            Ok(_) => panic!("wrong key must not succeed"),
            Err(CryptoError::KeyCommitmentFailed) => (),
            Err(e) => panic!("expected KeyCommitmentFailed, got {e:?}"),
        }
    }
    ...
    fn share_decryptor_happy_path() {
        ...
        let (mut dec, offset) =
            V7ShareDecryptor::new(&encrypted[..V7_HEADER_MIN], &ck_bytes).unwrap();
    ...
    fn share_decryptor_truncated_ciphertext_fails_finalize() {
        ...
        assert!(
            result.is_err(),
            "finalize must fail with truncated/wrong HMAC"
        );
    }
}
