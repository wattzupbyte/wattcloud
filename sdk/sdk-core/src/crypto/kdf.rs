...
mod tests {
    ...
    fn argon2id_rejects_memory_above_ceiling() {
        let salt = test_salt();
        let res = argon2id_derive_with_params(b"pw", &salt, ARGON2_MAX_MEMORY_KIB + 1, 3, 4);
        match res {
    ...
    fn argon2id_rejects_iterations_above_ceiling() {
        let salt = test_salt();
        let res = argon2id_derive_with_params(b"pw", &salt, 65536, ARGON2_MAX_ITERATIONS + 1, 4);
        match res {
            Err(CryptoError::Argon2ParamsOutOfBounds(m)) => {
    ...
    fn argon2id_rejects_parallelism_above_ceiling() {
        let salt = test_salt();
        let res = argon2id_derive_with_params(b"pw", &salt, 65536, 3, ARGON2_MAX_PARALLELISM + 1);
        match res {
            Err(CryptoError::Argon2ParamsOutOfBounds(m)) => {
