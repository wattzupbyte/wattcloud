...
mod tests {
    ...
    fn multiple_chunks_round_trips() {
        ...
        let plaintext: Vec<u8> = (0..2 * chunk_sz + 1337).map(|i| (i % 251) as u8).collect();
        let (mut flow, header, total_size) = ByoUploadFlow::new(
            &kp.mlkem_public_key,
            &kp.x25519_public_key,
            plaintext.len() as u64,
    ...
    fn exactly_one_full_chunk_round_trips() {
        ...
        let (mut flow, header, total_size) =
            ByoUploadFlow::new(&kp.mlkem_public_key, &kp.x25519_public_key, chunk_sz as u64)
                .unwrap();
    ...
    fn finalize_without_any_push_returns_ok_for_empty_file() {
        ...
        assert!(
            flow.finalize().is_ok(),
            "finalize() must succeed for empty files"
        );
    }
