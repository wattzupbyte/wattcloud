...
impl ByoDownloadFlow {
    ...
    pub fn push(&mut self, data: &[u8]) -> Result<Vec<u8>, SdkError> {
        ...
        if matches!(self.state, DownloadState::Header { .. }) {
            match std::mem::replace(&mut self.state, DownloadState::Done) {
                DownloadState::Header { buf, mlkem_sec, x25519_sec } => {
                    ...
                    let initial_plaintext =
                        decryptor.push(&released).map_err(SdkError::Crypto)?;

                    self.state = DownloadState::Body { trimmer, decryptor };
                    return Ok(initial_plaintext);
                }
                _ => return Err(SdkError::Crypto(CryptoError::InvalidFormat(
    ...
    pub fn finalize(mut self) -> Result<(), SdkError> {
        match std::mem::replace(&mut self.state, DownloadState::Done) {
            DownloadState::Body { trimmer, mut decryptor } => {
                let (remaining_body, footer) =
                    trimmer.finalize().map_err(SdkError::Crypto)?;
...
mod tests {
    ...
    fn make_v7_blob(plaintext_chunks: &[&[u8]]) -> (Vec<u8>, crate::crypto::zeroize_utils::HybridKeypair) {
        let kp = generate_hybrid_keypair().unwrap();
        let blob =
            encrypt_file_v7(&kp.mlkem_public_key, &kp.x25519_public_key, plaintext_chunks)
                .unwrap();
        (blob, kp)
    }
    ...
    fn tampered_hmac_detected_at_finalize() {
        ...
        assert!(
            matches!(result, Err(SdkError::Crypto(CryptoError::MacVerificationFailed))),
    ...
    fn wrong_key_returns_crypto_error() {
        ...
        let mut flow =
            ByoDownloadFlow::new(wrong_kp.mlkem_secret_key, wrong_kp.x25519_secret_key);
