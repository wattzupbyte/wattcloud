...
fn validate_device_id(id: &str) -> Result<(), StatsError> {
    ...
    for (i, &b) in bytes.iter().enumerate() {
        if hyphen_positions.contains(&i) {
            if b != b'-' { return Err(StatsError::InvalidDeviceId); }
...
impl<C: ProviderHttpClient> StatsUploader<C> {
    ...
    pub fn flush_batch(
        ...
        async move {
            ...
            let resp = self.client.request(req).await.map_err(|_| StatsError::Network)?;
            match resp.status {
                204 => Ok(()),
...
mod tests {
    ...
    impl ProviderHttpClient for MockClient {
        fn request(&self, _req: ProviderHttpRequest) -> impl Future<Output = Result<ProviderHttpResponse, ProviderError>> {
            let status = self.0;
            async move {
                Ok(ProviderHttpResponse { status, headers: vec![], body: vec![] })
            }
        }
    }
    ...
    async fn flush_204_ok() {
        ...
        up.flush_batch("5f3b1234-1234-1234-1234-1234567890ab", one_event(), "").await.unwrap();
    }

    #[tokio::test]
    async fn flush_401_unauthorized() {
        let up = StatsUploader::new(MockClient(401), "https://relay.example.com");
        let err = up.flush_batch("5f3b1234-1234-1234-1234-1234567890ab", one_event(), "relay_auth=x").await.unwrap_err();
    ...
    async fn flush_413_too_large() {
        ...
        let err = up.flush_batch("5f3b1234-1234-1234-1234-1234567890ab", one_event(), "").await.unwrap_err();
    ...
    async fn empty_events_skip_request() {
        ...
        up.flush_batch("5f3b1234-1234-1234-1234-1234567890ab", vec![], "").await.unwrap();
    }
