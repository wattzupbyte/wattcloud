...
impl StatsSink for NoopStatsSink {
    ...
    fn drain(&self, _max: usize) -> Vec<StatsEvent> { vec![] }
    fn len(&self) -> usize { 0 }
}
...
impl StatsSink for InMemoryStatsSink {
    ...
    fn drain(&self, max: usize) -> Vec<StatsEvent> {
        let Ok(mut q) = self.inner.lock() else { return vec![] };
...
mod tests {
    ...
    fn unlock(ts: u64) -> StatsEvent { StatsEvent::VaultUnlock { ts } }
    ...
    fn in_memory_drop_oldest_on_overflow() {
        ...
        let timestamps: Vec<u64> = drained.iter().map(|e| match e {
    ...
    fn drain_returns_at_most_max() {
        ...
        for i in 0..50 { s.record(unlock(i)); }
        let d = s.drain(5);
        assert_eq!(d.len(), 5);
        assert_eq!(s.len(), 45);
    }
