...
impl DeviceSlot {
    ...
    pub fn active(device_id: [u8; 16], wrap_iv: [u8; 12], encrypted_payload: [u8; 48]) -> Self {
        Self {
...
impl VaultHeader {
    ...
    fn parse_v2(data: &[u8]) -> Result<Self, VaultError> {
        ...
        let format_version = u16::from_le_bytes([data[8], data[9]]);
        let argon2_memory_kb =
            u32::from_le_bytes(data[10..14].try_into().map_err(|_| {
                VaultError::InvalidHeader("argon2_memory_kb parse failed".to_string())
            })?);
        let argon2_iterations = u32::from_le_bytes(data[14..18].try_into().map_err(|_| {
            VaultError::InvalidHeader("argon2_iterations parse failed".to_string())
        })?);
        let argon2_parallelism = u32::from_le_bytes(data[18..22].try_into().map_err(|_| {
            VaultError::InvalidHeader("argon2_parallelism parse failed".to_string())
        })?);
        ...
        let revocation_epoch = u32::from_le_bytes(
            data[VAULT_REVOCATION_EPOCH_OFFSET..VAULT_REVOCATION_EPOCH_OFFSET + 4]
                .try_into()
                .map_err(|_| {
                    VaultError::InvalidHeader("revocation_epoch parse failed".to_string())
                })?,
        );
    ...
    pub fn parse_v1(data: &[u8]) -> Result<Self, VaultError> {
        ...
        for i in 0..VAULT_MAX_DEVICES {
            ...
            let slot =
                DeviceSlot::from_bytes_v1(&data[offset..offset + VAULT_DEVICE_SLOT_SIZE_V1])?;
            device_slots_vec.push(slot);
        }
        let device_slots: [DeviceSlot; VAULT_MAX_DEVICES] =
            device_slots_vec.try_into().map_err(|_: Vec<_>| {
...
mod tests {
    ...
    fn make_valid_v1_header_bytes() -> Vec<u8> {
        ...
        buf[0..8].copy_from_slice(VAULT_MAGIC);
        buf[8..10].copy_from_slice(&VAULT_FORMAT_VERSION_V1.to_le_bytes());
        // Argon2 params
    ...
    fn device_slot_with_signing_key() {
        let slot = DeviceSlot::active_with_signing_key([1u8; 16], [2u8; 12], [3u8; 48], [4u8; 48]);
        assert!(slot.has_signing_key());
    }
