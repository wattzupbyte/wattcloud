...
impl SecretConfigJson {
    ...
    pub fn duplicate(&self) -> Self { Self(self.0.clone()) }
}
...
impl Manifest {
    ...
    pub fn primary_provider(&self) -> Option<&ManifestEntry> {
        self.providers
            .iter()
            .find(|e| !e.tombstone && e.is_primary)
    }
...
fn enforce_primary_uniqueness(providers: &mut [ManifestEntry]) -> Result<(), ManifestError> {
    ...
    if primary_indices.len() <= 1 {
        if primary_indices.is_empty() {
                ...
                .map(|(i, _)| i)
            {
                providers
                    .get_mut(idx)
                    .ok_or_else(|| ManifestError::InvariantViolated(
                        "index out of bounds (no-primary)".into(),
                    ))?
                    .is_primary = true;
            }
        }
    ...
    let winner_idx = primary_indices
        ...
        .max_by(|&a, &b| {
            ...
            let pa = providers.get(a).map(|e| e.provider_id.as_str()).unwrap_or("");
            let pb = providers.get(b).map(|e| e.provider_id.as_str()).unwrap_or("");
    ...
    for &idx in &primary_indices {
        providers
            .get_mut(idx)
            .ok_or_else(|| ManifestError::InvariantViolated(
                "index out of bounds (clear primary)".into(),
            ))?
            .is_primary = false;
    }
    providers
        .get_mut(winner_idx)
        .ok_or_else(|| ManifestError::InvariantViolated(
            "winner index out of bounds".into(),
        ))?
        .is_primary = true;
...
mod tests {
    ...
    fn merge_tombstone_wins_on_tie() {
        ...
        let merged = merge_manifests(&[&m1, &m2], u64::MAX, None).unwrap();
        assert!(merged.providers[0].tombstone, "tombstone should win the tie");
    }
    ...
    fn merge_newer_live_beats_older_tombstone() {
        ...
        let merged = merge_manifests(&[&m1, &m2], u64::MAX, None).unwrap();
        assert!(!merged.providers[0].tombstone, "newer live should beat older tombstone");
    }

    #[test]
    fn merge_commutativity() {
        let m1 = make_manifest(3, vec![make_entry("a", true, 2000), make_entry("b", false, 1000)]);
        let m2 = make_manifest(5, vec![make_entry("b", false, 1500), make_entry("c", false, 500)]);
    ...
    fn merge_multiple_primary_claims_resolved() {
        ...
        let primaries: Vec<_> = merged
            ...
            .collect();
        assert_eq!(primaries.len(), 1);
        assert_eq!(primaries[0].provider_id, "a", "provider with later updated_at should be primary");
    }
    ...
    fn validate_duplicate_provider_id_fails() {
        let m = make_manifest(1, vec![make_entry("dup", true, 1000), make_entry("dup", false, 2000)]);
        assert!(validate_manifest(&m, u64::MAX).is_err());
    }

    #[test]
    fn validate_multiple_primary_fails() {
        let m = make_manifest(1, vec![make_entry("a", true, 1000), make_entry("b", true, 2000)]);
        assert!(validate_manifest(&m, u64::MAX).is_err());
    }

    #[test]
    fn validate_no_primary_with_active_entries_fails() {
        let m = make_manifest(1, vec![make_entry("a", false, 1000), make_entry("b", false, 2000)]);
        assert!(validate_manifest(&m, u64::MAX).is_err());
    }
    ...
    fn merge_three_manifests_associativity() {
        ...
        let m2 = make_manifest(5, vec![make_entry("b", false, 1500), make_entry("a", false, 1000)]);
        let m3 = make_manifest(7, vec![make_entry("c", false, 500), make_entry("b", false, 2500)]);
        ...
        let mut ids1: Vec<_> = abc.providers.iter().map(|e| e.provider_id.as_str()).collect();
        let mut ids2: Vec<_> = abc2.providers.iter().map(|e| e.provider_id.as_str()).collect();
        ids1.sort();
        ids2.sort();
        assert_eq!(ids1, ids2, "3-manifest merge must be associative");

        // Primary must agree.
        let p1 = abc.providers.iter().find(|e| e.is_primary && !e.tombstone).map(|e| e.provider_id.as_str());
        let p2 = abc2.providers.iter().find(|e| e.is_primary && !e.tombstone).map(|e| e.provider_id.as_str());
        assert_eq!(p1, p2, "primary must agree across associative merge orderings");
    }
    ...
    fn provider_id_rejects_nul() {
        ...
        let m = make_manifest(1, vec![e]);
        assert!(validate_manifest(&m, u64::MAX).is_err(), "NUL byte in provider_id must be rejected");
    }
    ...
    fn provider_id_rejects_too_long() {
        ...
        let m = make_manifest(1, vec![e]);
        assert!(validate_manifest(&m, u64::MAX).is_err(), "provider_id > 64 bytes must be rejected");
    }
}
