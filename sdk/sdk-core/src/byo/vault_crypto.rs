...
pub fn migrate_vault_v1_to_v2(
    ...
    let mut header = VaultHeader::parse(
        &vault_bytes[..VAULT_HEADER_SIZE_V1.max(vault_bytes.len().min(VAULT_HEADER_SIZE))],
    )
    ...
    let v1_prefix = vault_bytes
        .get(..VAULT_HMAC_OFFSET_V1)
        .ok_or_else(|| CryptoError::InvalidFormat("v1 header too short to verify HMAC".into()))?;
    ...
    let body = vault_bytes.get(VAULT_HEADER_SIZE_V1..).ok_or_else(|| {
        CryptoError::InvalidFormat("v1 vault shorter than VAULT_HEADER_SIZE_V1 after parse".into())
    })?;
...
mod tests {
    ...
    fn migrate_v1_to_v2_produces_valid_header() {
        use crate::byo::vault_format::VaultHeader;
        use crate::crypto::constants::{VAULT_FORMAT_VERSION, VAULT_HEADER_SIZE};
