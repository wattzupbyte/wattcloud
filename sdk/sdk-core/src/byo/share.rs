...
// ─── Argon2id parameters ──────────────────────────────────────────────────────
//
// Matched to BYO vault KDF (ARGON2_MEMORY_KB_BYO = 131072, t=3, p=4) so the
// share-password derivation has the same security margin as the vault
// passphrase. Recipients pay ~2x longer unwrap on low-end devices (~4s on
// modern phones); the extra cost buys identical GPU-resistance.

const SHARE_ARGON2_MEMORY_KB: u32 = 131072; // 128 MB — matches BYO vault
...
pub fn unwrap_key_with_password(
    ...
    // Decode encrypted_ck.
    let enc_bytes = URL_SAFE_NO_PAD.decode(encrypted_ck_b64url).map_err(|_| {
        ShareError::InvalidFragment("encrypted_ck base64url decode failed".to_string())
    })?;
    ...
    let plaintext =
        aes_gcm_decrypt(ciphertext, &nonce, &share_key).map_err(|_| ShareError::WrongPassword)?;
