...
mod tests {
    ...
    fn unlock_all_online() {
        let plan = UnlockPlan::build(&["a", "b", "c"], &["a", "b", "c"], &[], "a", &[]);
        assert!(!plan.fail_closed);
        assert_eq!(plan.vault_steps.len(), 3);
        assert!(plan
            .vault_steps
            .iter()
            .all(|s| s.source == VaultBodySource::Cloud));
    }
    ...
    fn unlock_one_offline_no_cache_skipped() {
        ...
        assert!(!plan.fail_closed);
        assert_eq!(
            plan.vault_steps.len(),
            1,
            "unreachable+uncached provider should be skipped"
        );
