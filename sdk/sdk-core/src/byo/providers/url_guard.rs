...
pub fn validate_response_url(
    ...
    if !allowed_suffixes.is_empty() {
        ...
        let matches_suffix = allowed_suffixes
            .iter()
            .any(|suf| host_str == suf.to_ascii_lowercase()
...
/// (`serverUrl` for WebDAV, `s3_endpoint` for S3). Unlike
/// [`validate_response_url`], allows `http://` only when the host resolves to
/// a non-routable address (so test rigs / on-prem plaintext stays usable),
/// and always blocks private/internal IPs otherwise.
pub fn validate_config_url(url_str: &str) -> Result<Url, ProviderError> {
    ...
    // For config URLs we still prefer HTTPS, but allow http: only over loopback
    // (so Docker-compose/MinIO dev setups keep working). This is a config
    // value the vault owner chose; the response-URL guard is stricter.
    if url.scheme() != "https" && url.scheme() != "http" {
        return Err(ProviderError::Provider(format!(
            "refusing URL with scheme {}",
            url.scheme()
        )));
    }
    reject_private_host(&url)?;
...
mod tests {
    ...
    fn rejects_embedded_credentials() {
        assert!(
            validate_response_url("https://attacker:token@www.googleapis.com/", &[]).is_err()
        );
    }
    ...
    fn suffix_allowlist_enforced() {
        // Host suffix allowed.
        assert!(
            validate_response_url("https://x.googleusercontent.com/f", &["googleusercontent.com"])
                .is_ok()
        );
        // Exact match also allowed.
        assert!(
            validate_response_url("https://googleusercontent.com/f", &["googleusercontent.com"])
                .is_ok()
        );
