...
/// Build the full path for a Dropbox file (always starts with "/SecureCloud/").
...
async fn dbx_upload_call<H: ProviderHttpClient>(
    ...
    let arg_str =
        serde_json::to_string(&api_arg).map_err(|_| ProviderError::InvalidResponse)?;
...
async fn dbx_upload_session_start<H: ProviderHttpClient>(
    ...
    let req = ProviderHttpRequest::post(format!("{CONTENT_BASE}/files/upload_session/start"))
        .header(bearer(token))
        .header(("Dropbox-API-Arg".to_string(), r#"{"close":false}"#.to_string()))
        .header(("Content-Type".to_string(), "application/octet-stream".to_string()))
        .body(vec![]);
    let resp = http.request(req).await?;
    if let Some(e) = map_http_status(resp.status, &resp.body) {
...
async fn dbx_upload_session_append<H: ProviderHttpClient>(
    ...
    let req =
        ProviderHttpRequest::post(format!("{CONTENT_BASE}/files/upload_session/append_v2"))
            .header(bearer(token))
            .header(("Dropbox-API-Arg".to_string(), arg_str))
            .header(("Content-Type".to_string(), "application/octet-stream".to_string()))
            .body(data);
...
async fn dbx_upload_session_finish<H: ProviderHttpClient>(
    ...
    let arg_str = serde_json::to_string(&arg).map_err(|_| ProviderError::InvalidResponse)?;
    let req =
        ProviderHttpRequest::post(format!("{CONTENT_BASE}/files/upload_session/finish"))
            .header(bearer(token))
            .header(("Dropbox-API-Arg".to_string(), arg_str))
            .header(("Content-Type".to_string(), "application/octet-stream".to_string()))
            .body(final_chunk);
...
impl<H: ProviderHttpClient + Send + Sync + 'static> StorageProvider for DropboxProvider<H> {
    ...
    fn refresh_auth(
        ...
        async move {
            ...
            let client_id = client_id.ok_or(ProviderError::Provider("client_id required for refresh".into()))?;
    ...
    fn upload(
        ...
        async move {
            ...
            // Use the ref_ (path) if provided, otherwise build path under SecureCloud/
            let path = match &ref_ {
    ...
    fn list(
        ...
        async move {
            ...
            let path = parent_ref.unwrap_or_else(|| "/SecureCloud".to_string());
...
mod tests {
    ...
    async fn upload_returns_ref_and_version() {
        ...
        let result = p
            .upload(None, "test.bin".into(), b"hello".to_vec(), UploadOptions::default())
            .await
            .unwrap();
    ...
    fn session_start_resp() -> Vec<u8> {
        serde_json::json!({ "session_id": "sess_abc123" }).to_string().into_bytes()
    }
    ...
    async fn stream_small_file_no_append() {
        ...
        let sid = p
            ...
            .await
            .unwrap();
        p.upload_stream_write(sid.clone(), b"hel".to_vec()).await.unwrap();
        p.upload_stream_write(sid.clone(), b"lo!".to_vec()).await.unwrap();
    ...
    async fn stream_large_file_with_append() {
        ...
        let sid = p
            .upload_stream_open(None, "huge.bin".into(), chunk.len() as u64, UploadOptions::default())
            .await
            .unwrap();
    ...
    async fn stream_abort_cleans_state() {
        ...
        assert!(p.state.lock().unwrap().upload_sessions.get(&sid).is_none());
    }
    ...
    async fn dropbox_download_range_headers_correctly_formed() {
        ...
        let http = Arc::new(RecordingHttp::new(vec![
            (206, b"chunk".to_vec(), vec![
                ("Content-Range".to_string(), "bytes 0-4/5".to_string()),
            ]),
        ]));
        ...
        assert!(req.url.ends_with("/2/files/download"), "url was: {}", req.url);
        ...
        assert!(api_arg.contains(ref_path), "Dropbox-API-Arg did not contain path: {api_arg}");
        // Range header specifies the chunk.
        let range = header("range").expect("range header missing");
        assert!(range.starts_with("bytes=0-"), "range should start at byte 0: {range}");
    }
}
