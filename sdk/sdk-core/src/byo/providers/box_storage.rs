...
//   - SecureCloud folder: created under Box root (id="0") at init(); ID cached.
...
struct BoxState {
    ...
    /// Cached Box item ID of the "SecureCloud" folder.
...
/// Find or create the "SecureCloud" folder directly inside Box root (id="0").
        ...
        entries
            ...
            .find(|e| {
                e.get("type").and_then(|t| t.as_str()) == Some("folder")
                    && e.get("name").and_then(|n| n.as_str()) == Some("SecureCloud")
            })
            .map(parse_box_id)
...
async fn box_upload_file<H: ProviderHttpClient>(
    ...
    if resp.status == 409 {
        ...
        let version = parse_json(&resp.body)
            ...
            .unwrap_or_default();
        return Err(ProviderError::Conflict { current_version: version });
    }
    if let Some(e) = map_http_status(resp.status, &resp.body) {
        return Err(e);
    }
    ...
    let file_obj = if let Some(entries) = meta.get("entries").and_then(|e| e.as_array()) {
        entries.first().ok_or(ProviderError::InvalidResponse)?.clone()
    } else {
        meta
    };
...
impl<H: ProviderHttpClient + Send + Sync + 'static> StorageProvider for BoxProvider<H> {
    ...
    fn init(
        ...
        async move {
            ...
            // Eagerly ensure the SecureCloud folder exists.
    ...
    fn upload(
        ...
        async move {
            ...
            // Resolve or re-fetch SecureCloud folder ID.
    ...
    fn upload_stream_open(
        ...
        async move {
            ...
            let existing_file_id =
                ref_.as_deref().filter(|r| !r.is_empty()).map(|s| s.to_string());
    ...
    fn list(
        ...
        async move {
            ...
            for item in entries {
                let is_folder =
                    item.get("type").and_then(|t| t.as_str()) == Some("folder");
                let name = item
                    .get("name")
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
