...
pub(crate) fn map_http_status(status: u16, body: &[u8]) -> Option<ProviderError> {
    match status {
        ...
        409 | 412 => {
            ...
            Some(ProviderError::Conflict { current_version: version })
        }
        _ => {
            let msg = String::from_utf8_lossy(body).chars().take(200).collect::<String>();
...
pub(crate) fn extract_dropbox_conflict_rev(body: &[u8]) -> Option<String> {
    ...
    v.get("error_summary").and_then(|s| s.as_str()).map(|s| s.to_string())
}
...
pub(crate) fn extract_box_conflict_etag(body: &[u8]) -> Option<String> {
    ...
    conflicts.get("etag").and_then(|e| e.as_str()).map(|s| s.to_string())
}
...
pub(crate) type MakeHeaders =
    Arc<dyn Fn(u64, u64) -> Vec<(String, String)> + Send + Sync>;
...
pub(crate) type HttpCallFn = Arc<
    dyn Fn(ProviderHttpRequest) -> Pin<Box<dyn Future<Output = Result<ProviderHttpResponse, ProviderError>> + Send>>
        + Send
        + Sync,
>;
...
pub(crate) type HttpCallFn = Arc<
    dyn Fn(ProviderHttpRequest) -> Pin<Box<dyn Future<Output = Result<ProviderHttpResponse, ProviderError>>>>
        + Send
        + Sync,
>;
...
impl RangedDownloadBuffer {
    pub fn new(
        ...
        Self {
            url, method, fixed_body, make_headers, http_call,
            offset: 0, total_size: 0, chunk_size: RANGE_CHUNK_SIZE,
            done: false,
        }
    }
    ...
    pub fn apply_response(
        ...
        match status {
            ...
            _ => Err(map_http_status(status, &body)
                .unwrap_or_else(|| ProviderError::Provider(format!("range request failed: HTTP {status}")))),
        }
    }
}
...
mod tests {
    ...
    fn dummy_http_call() -> HttpCallFn {
        Arc::new(|_req: ProviderHttpRequest| {
            Box::pin(async {
                Err(ProviderError::Provider("no-op mock http".to_string()))
            })
        })
    }
    ...
    fn parse_content_range_total_standard_form() {
        assert_eq!(parse_content_range_total("bytes 0-8388607/93547009"), Some(93547009));
    }
    ...
    fn apply_response_206_updates_total_from_content_range() {
        let mut buf = RangedDownloadBuffer::new(
            "http://x".to_string(), "GET", None, dummy_make_headers(), dummy_http_call(),
        );
        let body = vec![0xAA; 100];
        let result = buf.apply_response(206, body.clone(), Some("bytes 0-99/5000"), 100).unwrap();
    ...
    fn apply_response_206_second_chunk_preserves_total() {
        ...
        buf.apply_response(206, vec![0; 100], Some("bytes 0-99/300"), 100).unwrap();
    ...
    fn apply_response_206_short_read_without_content_range_marks_done() {
        ...
        buf.apply_response(206, vec![0; 4_000_000], None, 8_000_000).unwrap();
        assert_eq!(buf.offset, 4_000_000);
        assert!(buf.is_done(), "short-read without Content-Range should mark done");
        assert!(buf.next_request().is_none());
    }
    ...
    fn next_request_builds_correct_range_header() {
        let headers: MakeHeaders = Arc::new(|offset, end| {
            vec![("range".to_string(), format!("bytes={offset}-{end}"))]
        });
        let buf = RangedDownloadBuffer::new(
            "http://x".to_string(), "GET", None, headers, dummy_http_call(),
        );
        let (req, requested) = buf.next_request().unwrap();
        assert_eq!(req.method, "GET");
        assert_eq!(req.url, "http://x");
        assert_eq!(req.headers[0], ("range".to_string(), format!("bytes=0-{}", RANGE_CHUNK_SIZE - 1)));
    ...
    fn next_request_caps_end_at_total_size_minus_one() {
        ...
        let (req, requested) = buf.next_request().unwrap();
        assert_eq!(req.headers[0], ("range".to_string(), "bytes=100-299".to_string()));
