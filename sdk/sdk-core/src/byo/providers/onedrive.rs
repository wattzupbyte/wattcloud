...
/// Build the Graph API path for a file under WattcloudVault/.
...
/// Ensure the /WattcloudVault folder exists. Returns its item ID.
#[allow(dead_code)]
async fn od_ensure_root<H: ProviderHttpClient>(
    ...
    let result = od_get_item(http, token, "/WattcloudVault").await;
...
fn parse_modified_at(meta: &serde_json::Value) -> Option<i64> {
    let s = meta.get("lastModifiedDateTime").and_then(|v| v.as_str())?;
        ...
        async move {
            ...
            loop {
                let (upload_url, range_start, total_size, payload, is_final) = {
                    ...
                    let remaining_after_chunk = session
                        .total_size
                        .saturating_sub(session.offset.saturating_add(OD_CHUNK_SIZE as u64));
                    let is_final =
                        remaining_after_chunk == 0 && session.buffer.len() == OD_CHUNK_SIZE;
                ...
                let resp =
                    od_put_range(&*http, &upload_url, range_start, total_size, payload).await?;
    ...
    fn list(
        ...
        async move {
            ...
            let url = match parent_ref {
                ...
                None => format!("{}/root:/WattcloudVault:/children", GRAPH_BASE),
            };
            let req = ProviderHttpRequest::get(url).header(bearer(&token));
            let resp = http.request(req).await?;
            if let Some(e) = map_http_status(resp.status, &resp.body) {
...
mod tests {
    ...
    async fn upload_stream_abort_issues_delete_to_upload_url() {
        ...
        .to_string()
        .into_bytes();
        let http = RecordingHttp::new(vec![(200, session_resp, vec![]), (204, vec![], vec![])]);
    ...
    async fn resumable_upload_roundtrip() {
        ...
        let session_resp =
            serde_json::json!({ "uploadUrl": "https://example.sharepoint.com/session1" })
                .to_string()
                .into_bytes();
        ...
        p.upload_stream_write(sid.clone(), b"hello".to_vec())
            .await
            .unwrap();
        p.upload_stream_write(sid.clone(), b" yoo".to_vec())
            .await
            .unwrap();
