...
const SECURECLOUD_FOLDER: &str = "SecureCloud";
...
impl<H: ProviderHttpClient + Send + Sync + 'static> StorageProvider for GdriveProvider<H> {
    ...
    fn init(
        ...
        async move {
            let token = config.access_token.clone().ok_or(ProviderError::Unauthorized)?;
            {
    ...
    fn upload(
        ...
        async move {
            ...
            gdrive_upload(&*http, &token, ref_, &name, &data, &options, root_folder_id.as_deref())
                .await
        }
    }
    ...
    fn get_version(
        ...
        async move {
            ...
            Ok(json.get("etag").and_then(|e| e.as_str()).unwrap_or("").to_string())
        }
    }
    ...
    fn upload_stream_write(
        ...
        async move {
            let (session_uri, total_size, start) = {
                ...
                (sess.session_uri.clone(), sess.total_size, sess.bytes_written)
            };
            ...
            let resp = http
                .request(
                    ProviderHttpRequest::put(session_uri)
                        .body(chunk)
                        .header(("Content-Range".to_string(), format!("bytes {start}-{end}/{total_size}"))),
                )
                .await?;
            ...
            let terminal_result = if resp.status == 200 || resp.status == 201 {
                ...
                let etag = json.get("etag").and_then(|e| e.as_str()).unwrap_or("").to_string();
                Some(UploadResult { ref_: file_id, version: etag })
    ...
    fn upload_stream_close(
        ...
        async move {
            ...
            let file_id = json_str(&json, "id")?.to_string();
            let etag = json.get("etag").and_then(|e| e.as_str()).unwrap_or("").to_string();
            Ok(UploadResult { ref_: file_id, version: etag })
        }
    }
        ...
        async move {
            ...
            for f in files {
                ...
                let size = f
                    ...
                    .unwrap_or(0);
                let mime =
                    f.get("mimeType").and_then(|m| m.as_str()).unwrap_or("").to_string();
...
async fn gdrive_ensure_root_folder<H: ProviderHttpClient>(
    ...
    let query = format!(
        "name='{SECURECLOUD_FOLDER}' and mimeType='{GDRIVE_FOLDER_MIME}' and trashed=false and 'root' in parents"
    );
    let resp = http
        .request(
...
async fn gdrive_upload<H: ProviderHttpClient>(
    ...
    if let Some(id) = &ref_ {
        ...
        return Ok(UploadResult {
            ...
            version: json.get("etag").and_then(|e| e.as_str()).unwrap_or("").to_string(),
        });
    }
...
async fn gdrive_start_resumable_upload<H: ProviderHttpClient>(
    ...
    .body(body_bytes)
    .header(bearer(token))
    .header(("X-Upload-Content-Type".to_string(), "application/octet-stream".to_string()))
    .header(("X-Upload-Content-Length".to_string(), total_size.to_string()));
    if has_json_body {
        req = req.header(("Content-Type".to_string(), "application/json; charset=UTF-8".to_string()));
    }
    if let Some(ver) = &options.expected_version {
        req = req.header(("If-Match".to_string(), ver.clone()));
    }
...
mod tests {
    ...
    async fn init_unauthorized_returns_error() {
        ...
        let p = GdriveProvider::new(http);
        assert!(matches!(p.init(cfg("expired")).await, Err(ProviderError::Unauthorized)));
    }
    ...
    async fn upload_new_file_multipart() {
        ...
        p.init(cfg("tok")).await.unwrap();
        let r = p
            .upload(None, "test.v7".to_string(), b"hello".to_vec(), UploadOptions::default())
            .await
            .unwrap();
    ...
    async fn upload_conflict_on_412() {
        ...
        let err = p
            .upload(
                ...
                "f.v7".to_string(),
                b"d".to_vec(),
                UploadOptions { expected_version: Some("v1".to_string()), ..Default::default() },
            )
            .await
            .unwrap_err();
        assert!(matches!(err, ProviderError::Conflict { current_version } if current_version == "v2"));
    }
    ...
    async fn upload_stream_roundtrip() {
        let http = MockHttp::with_headers(vec![
            ...
            (200, r#"{}"#, vec![("Location", "https://up.googleapis.com/sess")]),
            (308, "", vec![]),
        ...
        let sid = p
            ...
            .await
            .unwrap();
        p.upload_stream_write(sid.clone(), vec![0u8; 512]).await.unwrap();
    ...
    impl RangeServingHttp {
        fn new(
            init_responses: Vec<(u16, &str, Vec<(&str, &str)>)>,
            body: Vec<u8>,
        ) -> Self {
            Self {
                init_responses: Mutex::new(
                    init_responses.into_iter()
    ...
    impl ProviderHttpClient for RangeServingHttp {
        ...
        ) -> impl std::future::Future<Output = Result<ProviderHttpResponse, ProviderError>> + Send
        {
            ...
            if !init.is_empty() {
                ...
                return std::future::ready(Ok(ProviderHttpResponse { status, headers, body }));
            }
            drop(init);

            // After init, serve Range requests from the stored body.
            let range_hdr = req.headers.iter()
            ...
            let result = match range_hdr.as_deref() {
                Some(r) => {
                    ...
                    if start >= total {
                        ProviderHttpResponse { status: 416, headers: vec![], body: vec![] }
                    } else {
    ...
    async fn gdrive_download_stream_e2e_v7_roundtrip_via_range() {
        ...
        let v7_bytes = encrypt_file_v7(&kp.mlkem_public_key, &kp.x25519_public_key, &chunks).unwrap();
        ...
        loop {
            match p.download_stream_read(sid.clone()).await.unwrap() {
                Some(chunk) => downloaded.extend_from_slice(&chunk),
                None => break,
            }
        }
        p.download_stream_close(sid).await.unwrap();

        assert_eq!(downloaded, v7_bytes,
        ...
        let recovered = decrypt_file_v7(&downloaded, &kp.mlkem_secret_key, &kp.x25519_secret_key).unwrap();
    ...
    async fn download_stream_multi_chunk_via_range() {
        ...
        let http = MockHttp::with_headers(vec![
            ...
            (206, "chunk2_data", vec![("Content-Range", "bytes 11-21/33")]),
            (206, "chunk3_final", vec![("Content-Range", "bytes 22-32/33")]),
        ]);
        let p = GdriveProvider::new(http);
        p.init(cfg("tok")).await.unwrap();
        let sid = p.download_stream_open("f".to_string()).await.unwrap();
