...
//   - Refs: absolute file PATHS (e.g. `/SecureCloud/data/file.bin`).
...
//   - SecureCloud root: `/SecureCloud` — created at init().
...
const PCLOUD_ROOT: &str = "/SecureCloud";

// ─── State ────────────────────────────────────────────────────────────────────

/// In-progress buffer-then-upload session.
struct UploadBuffer {
    /// Absolute destination path of the file (e.g. `/SecureCloud/data/file.bin`).
    path: String,
...
fn check_pcloud_result(v: &serde_json::Value) -> Result<(), ProviderError> {
    ...
    match code {
        ...
        _ => {
            ...
            Err(ProviderError::Provider(format!("pCloud error {code}: {msg}")))
        }
    }
}

/// Ensure the SecureCloud root folder exists.
...
/// `path` must be an absolute path like `/SecureCloud/data/file.bin`.
async fn pcloud_upload<H: ProviderHttpClient>(
    ...
    if let Some(expected) = expected_version {
        ...
        match meta {
            Ok(m) => {
                ...
                if current != expected {
                    return Err(ProviderError::Conflict { current_version: current });
                }
            }
    ...
    Ok(UploadResult { ref_: file_path, version })
}
...
impl<H: ProviderHttpClient + Send + Sync + 'static> StorageProvider for PCloudProvider<H> {
    ...
    fn download(
        ...
        async move {
            ...
            let link_url =
                format!("{}/getfilelink?path={}", base, percent_encode(&ref_));
            let req = ProviderHttpRequest::get(link_url).header(bearer(&token));
            let resp = http.request(req).await?;
            if let Some(e) = map_http_status(resp.status, &resp.body) {
                return Err(e);
        ...
        async move {
            ...
            let stream_id = new_stream_id();
            {
                ...
                s.download_refresh.insert(
                    stream_id.clone(),
                    PCloudDownloadRefresh { ref_: ref_.clone(), token, api_base: base },
                );
            }
            Ok(stream_id)
        }
        ...
        async move {
            ...
            if resp.status == 401 || resp.status == 403 {
                let refresh = {
                    ...
                    s.download_refresh.get(&stream_id).map(|r| PCloudDownloadRefresh {
                        ref_: r.ref_.clone(),
                        token: r.token.clone(),
                ...
                let fresh_url = fetch_pcloud_cdn_url(
                    &*http, &refresh.ref_, &refresh.token, &refresh.api_base,
                ).await?;
                ...
                return match s.download_buffers.get_mut(&stream_id) {
                    None => Ok(None),
                    Some(buf) => buf.apply_response(retry_resp.status, retry_resp.body, content_range.as_deref(), requested),
                };
            }
...
mod tests {
    ...
    async fn download_returns_body() {
        ...
        let data = p.download("/SecureCloud/file.bin".into()).await.unwrap();
        assert_eq!(data, b"file bytes");
    }

    #[tokio::test]
    async fn upload_stream_roundtrip() {
        let path = "/SecureCloud/big.bin";
        ...
        p.upload_stream_write(sid.clone(), b"world".to_vec()).await.unwrap();
    ...
    async fn download_stream_roundtrip() {
        ...
        let sid = p
            .download_stream_open("/SecureCloud/file.bin".into())
            .await
            .unwrap();
    ...
    async fn percent_encode_spaces_and_specials() {
        assert_eq!(percent_encode("/SecureCloud/my file.bin"), "/SecureCloud/my%20file.bin");
    ...
    async fn download_stream_auto_refreshes_cdn_url_on_403() {
        ...
        let sid = p.download_stream_open("/SecureCloud/file.bin".into()).await.unwrap();
        let chunk = p.download_stream_read(sid.clone()).await.unwrap();
        assert_eq!(chunk, Some(b"streamed-data".to_vec()), "retry after refresh must succeed");
        p.download_stream_close(sid).await.unwrap();
    }
    ...
    async fn download_stream_close_clears_refresh_state() {
        ...
        let s = p.state.lock().unwrap();
        assert!(!s.download_refresh.contains_key(&sid), "refresh state must be cleaned up");
    }
}
