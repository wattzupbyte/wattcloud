...
pub fn plan_reconcile(
    ...
    pending_steps
        .iter()
        .filter_map(
            |step| match decide_replay(step, dst_file_exists, src_blob_exists) {
                super::execution::ReplayDecision::Retry {
                    provider_id,
                    provider_ref,
                } => Some(ReconcileAction::DeleteBlob {
                    provider_id,
                    provider_ref,
                }),
                _ => None,
...
mod tests {
    ...
    fn multiple_pending_deletes_all_returned() {
        let steps = vec![del("gdrive", "data/aaa"), del("dropbox", "data/bbb")];
