...
mod tests {
    ...
    fn first_failure_transitions_to_offline_and_returns_true() {
        ...
        assert!(m.record_failure(), "first failure should signal status change");
    ...
    fn subsequent_failures_do_not_retransition() {
        let mut m = ProviderMonitor::new();
        m.record_failure();
        assert!(!m.record_failure(), "second failure should not signal again");
    ...
    fn success_after_failure_transitions_to_connected() {
        let mut m = ProviderMonitor::new();
        m.record_failure();
        assert!(m.record_success(), "success after failure should signal change");
        assert_eq!(m.status(), ProviderStatus::Connected);
    }
