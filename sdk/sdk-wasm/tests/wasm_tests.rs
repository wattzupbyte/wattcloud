...
fn test_encrypt_file_v7_wrong_key_fails() {
    ...
    let encrypted =
        encrypt_file_v7(b"secret data".to_vec(), pub_json).expect("encrypt must succeed");
...
fn test_encrypt_file_v7_large_payload() {
    ...
    let encrypted =
        encrypt_file_v7(plaintext.clone(), pub_json).expect("1MB encryption must succeed");

    let decrypted = decrypt_file_v7(encrypted, sec_json).expect("1MB decryption must succeed");
...
fn test_v7_stream_decryptor_round_trip() {
    ...
    let plaintext: Vec<u8> = (0..700 * 1024)
        ...
        .collect();
    let blob = encrypt_file_v7(plaintext.clone(), pub_json).expect("encrypt_file_v7 must succeed");
    assert!(
...
fn test_encrypt_filename_with_fresh_key_round_trip() {
    ...
    let key_bytes =
        decrypt_file_v7(encrypted_key_bytes, sec_json.clone()).expect("filename key unwrap failed");
    use base64::Engine;
...
fn test_encrypt_filename_with_fresh_key_with_metadata() {
    ...
    let encrypted_key_bytes = b64_decode_binary(&encrypted_key_b64);
    let key_bytes =
        decrypt_file_v7(encrypted_key_bytes, sec_json).expect("filename key unwrap failed");
    use base64::Engine;
    let key_b64 = base64::engine::general_purpose::STANDARD.encode(&key_bytes);
...
fn test_byo_encrypt_decrypt_body_roundtrip() {
    ...
    assert_eq!(
        decrypted, plaintext,
        "Decrypted body must match original plaintext"
    );
}
...
fn test_byo_decrypt_body_wrong_key_fails() {
    ...
    let nonce_and_ct = byo_encrypt_vault_body(plaintext, k1).expect("encrypt must succeed");
...
fn test_byo_enrollment_derive_session_both_sides_match() {
    ...
    let session_a = byo_enrollment_derive_session(
        get_str(&a, "eph_sk"),
        get_str(&b, "eph_pk"),
        channel_id.clone(),
    );
    let session_b =
        byo_enrollment_derive_session(get_str(&b, "eph_sk"), get_str(&a, "eph_pk"), channel_id);
...
fn test_byo_enrollment_derive_session_wrong_peer_differs() {
    ...
    let session_c =
        byo_enrollment_derive_session(get_str(&c, "eph_sk"), get_str(&b, "eph_pk"), channel_id);
...
fn test_byo_enrollment_encrypt_decrypt_shard_roundtrip() {
    ...
    let channel_id = get_str(&a, "channel_id");
    let session =
        byo_enrollment_derive_session(get_str(&a, "eph_sk"), get_str(&b, "eph_pk"), channel_id);
