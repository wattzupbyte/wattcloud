...
async fn dispatch_op<P: StorageProvider>(
    ...
    match op {
        ...
        "createPresignedUrl" => {
            ...
            let url = provider
                .create_presigned_url(ref_, ttl)
                .await
                .map_err(|e| JsValue::from_str(&e.to_string()))?;
            serde_json::to_string(&url).map_err(|e| JsValue::from_str(&e.to_string()))
        }
        _ => Err(JsValue::from_str(&format!(
            "byoProviderCall: unknown op: {op}"
        ))),
    }
}
...
pub async fn byo_provider_call(
    ...
    let config: ProviderConfig = serde_json::from_value(
        args.get("config")
            .cloned()
            .ok_or_else(|| JsValue::from_str("missing 'config' in args"))?,
    )
    .map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    match provider_type {
        "gdrive" => with_provider!(sdk_core::byo::GdriveProvider::new(
            ReqwestProviderHttpClient::new()
        )),
        "dropbox" => with_provider!(sdk_core::byo::DropboxProvider::new(
            ReqwestProviderHttpClient::new()
        )),
        "onedrive" => with_provider!(sdk_core::byo::OneDriveProvider::new(
            ReqwestProviderHttpClient::new()
        )),
        "webdav" => with_provider!(sdk_core::byo::WebDAVProvider::new(
            ReqwestProviderHttpClient::new()
        )),
        "box" => with_provider!(sdk_core::byo::BoxProvider::new(
            ReqwestProviderHttpClient::new()
        )),
        "pcloud" => with_provider!(sdk_core::byo::PCloudProvider::new(
            ReqwestProviderHttpClient::new()
        )),
        "s3" => with_provider!(sdk_core::byo::S3Provider::new(
            ReqwestProviderHttpClient::new()
        )),
        _ => Err(JsValue::from_str(&format!(
            "byoProviderCall: unknown provider_type: {provider_type}"
        ))),
    }
}
...
pub async fn byo_s3_list(config_json: &str, parent_ref: Option<String>) -> Result<String, JsValue> {
    let config: ProviderConfig =
        serde_json::from_str(config_json).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub async fn byo_s3_create_presigned_url(
    ...
    provider
        ...
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
    provider
        .create_presigned_url(ref_, ttl_seconds)
        .await
        .map_err(|e| JsValue::from_str(&e.to_string()))
}
...
pub async fn byo_cross_provider_stream_copy(
    ...
    let src_cfg: ProviderConfig =
        serde_json::from_str(src_config_json).map_err(|e| JsValue::from_str(&e.to_string()))?;
    let dst_cfg: ProviderConfig =
        serde_json::from_str(dst_config_json).map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    macro_rules! with_dst {
        ($src:expr) => {
            match dst_type {
                ...
                "onedrive" => {
                    let dst =
                        sdk_core::byo::OneDriveProvider::new(ReqwestProviderHttpClient::new());
                    dst.init(dst_cfg)
                        .await
                        .map_err(|e| JsValue::from_str(&e.to_string()))?;
                    pipe_between_providers(&$src, &dst, src_ref, dst_name, total_size).await
