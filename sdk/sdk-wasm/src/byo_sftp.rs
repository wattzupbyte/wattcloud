...
#[wasm_bindgen]
pub fn sftp_store_credential_publickey(private_key: String, passphrase: Option<String>) -> u32 {
    let handle = next_cred_handle();
    SFTP_CREDS.with(|m| {
        m.borrow_mut().insert(
            handle,
...
fn lookup_sftp_cred(handle: u32) -> Result<ExtractedCred, &'static str> {
    SFTP_CREDS.with(|m| {
        ...
        match m.get(&handle) {
            ...
            Some(SftpCred::PublicKey {
                private_key,
                passphrase,
            }) => Ok(ExtractedCred::PublicKey {
                private_key: Zeroizing::new((**private_key).clone()),
                passphrase: passphrase.as_ref().map(|p| Zeroizing::new((**p).clone())),
            }),
...
impl WasmRelayTransport {
    fn new(send_text_fn: Function, send_text_binary_fn: Function, close_fn: Function) -> Self {
        Self {
...
impl RelayTransport for WasmRelayTransport {
    fn send_text(&self, s: &str) -> impl std::future::Future<Output = Result<(), ProviderError>> {
        ...
        async move {
            r.map_err(|e| ProviderError::SftpRelay(format!("{e:?}")))
                .map(|_| ())
        }
    }
    ...
    fn send_text_then_binary(
        ...
        let r = self
            .send_text_binary_fn
            .call2(&JsValue::NULL, &JsValue::from_str(text_hdr), &arr);
        async move {
            r.map_err(|e| ProviderError::SftpRelay(format!("{e:?}")))
                .map(|_| ())
        }
...
impl SftpSessionWasm {
    ...
    /// - `close_fn`: `() => void`
    /// - `base_path`: optional server-absolute prefix applied to the vault
    ///   root (e.g. `"/wattcloud"` → vault lands at
    ///   `/wattcloud/WattcloudVault`).  Pass `None` or `""` for the default
    ///   `/WattcloudVault`.
    ...
    pub fn new(
        ...
        let client = Rc::new(SftpRelayClient::new(
            transport_ref,
        ...
        future_to_promise(async move {
            let version = client
                .handshake(|fp| {
                    ...
                    async move {
                        let result = cb
                            .call1(&JsValue::NULL, &JsValue::from_str(&fp))
    ...
    pub fn auth_publickey(
        ...
        future_to_promise(async move {
            client
                .auth_publickey(&username, &private_key, passphrase.as_deref())
                .await
                .map_err(|e| JsValue::from_str(&e.to_string()))?;
            Ok(JsValue::UNDEFINED)
        })
    ...
    /// Query free bytes on the remote file system via the relay's
    /// `statvfs@openssh.com` extension shim. Returns the number as a JS
    /// number when the server supports the extension, or `null` when it
    /// doesn't (non-OpenSSH servers) or the query failed — callers should
    /// skip their quota gate in both cases.
    pub fn fs_info(&self, path: String) -> js_sys::Promise {
        let client = Rc::clone(&self.client);
        future_to_promise(async move {
            let free = client
                .fs_info(&path)
                ...
                .map_err(|e| JsValue::from_str(&e.to_string()))?;
            match free {
                Some(bytes) => Ok(JsValue::from_f64(bytes as f64)),
    ...
    pub fn stat(&self, path: String) -> js_sys::Promise {
        let client = Rc::clone(&self.client);
        future_to_promise(async move {
            let (mtime, size, is_dir) = client
                .stat(&path)
                .await
                .map_err(|e| JsValue::from_str(&e.to_string()))?;
            Ok(JsValue::from_str(
                &serde_json::json!({"mtime":mtime,"size":size,"isDir":is_dir}).to_string(),
            ))
        })
    }
    ...
    pub fn upload_open(&self, name: String, total_size: f64) -> js_sys::Promise {
        ...
        future_to_promise(async move {
            let id = client
                .upload_open(&name, total_size as u64)
                .await
                .map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    pub fn upload_write_chunk(&self, stream_id: String, chunk: Uint8Array) -> js_sys::Promise {
        ...
        future_to_promise(async move {
            client
                .upload_write_chunk(&stream_id, &chunk)
                .await
                .map_err(|e| JsValue::from_str(&e.to_string()))?;
            Ok(JsValue::UNDEFINED)
        })
    ...
    // ── Streaming download (relay protocol v3) ────────────────────────────────

    /// Open a streaming read session. Returns an opaque handle string.
    pub fn read_open(&self, path: String) -> js_sys::Promise {
        let client = Rc::clone(&self.client);
        future_to_promise(async move {
            let handle = client
                .read_open(&path)
    ...
    /// Pull the next chunk. Resolves with `Uint8Array` (may be zero-length —
    /// the client ReadableStream wrapper treats empty as EOF) or with
    /// `null` at EOF.
    pub fn read_chunk(&self, handle: String) -> js_sys::Promise {
        let client = Rc::clone(&self.client);
        future_to_promise(async move {
            match client
                .read_chunk(&handle)
            ...
            {
                None => Ok(JsValue::NULL),
                Some(data) => Ok(Uint8Array::from(data.as_slice()).into()),
            }
        })
    }

    /// Close a streaming read session. Idempotent.
    pub fn read_close(&self, handle: String) -> js_sys::Promise {
        let client = Rc::clone(&self.client);
        future_to_promise(async move {
            client
                .read_close(&handle)
    ...
    /// Inject the TOFU fingerprint persisted in `ProviderConfig` before the
    /// next [`handshake`]. Without this call, the client treats every
    /// connection as a first-connect and delegates to the `onFirstHostKey`
    /// callback, silently defeating TOFU protection for returning users.
    pub fn set_stored_fingerprint(&self, fingerprint: String) {
        let _ = self.client.set_stored_fingerprint(fingerprint);
    }
    ...
    pub fn relay_bandwidth_and_reset(&self) -> JsValue {
        ...
        let _ = js_sys::Reflect::set(
            ...
            &JsValue::from_f64(sent as f64),
        );
        let _ = js_sys::Reflect::set(
            &obj,
            &JsValue::from_str("recv"),
            &JsValue::from_f64(recv as f64),
        );
        obj.into()
    }
}
