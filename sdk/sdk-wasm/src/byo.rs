...
use sdk_core::byo::{
    ...
    enrollment::{
        decrypt_shard_from_transfer, encrypt_shard_for_transfer, enrollment_derive_session,
        enrollment_initiate, EnrollmentSession, ShardEnvelope,
    },
    vault_crypto::{
        ...
        seal_device_signing_key, unseal_device_signing_key, unwrap_vault_key,
        verify_header_hmac, verify_header_hmac_v1, wrap_vault_key,
    },
    vault_format::VaultHeader,
};
use sdk_core::crypto::kdf::{argon2id_derive_with_params, hkdf_sha256};
use sdk_core::crypto::symmetric::{aes_gcm_decrypt, generate_aes_key};
use sdk_core::crypto::wire_format::{decrypt_file_v7_init, V7ShareDecryptor};
    ...
    match enrollment_initiate() {
        Ok((eph_sk, eph_pk, channel_id)) => {
            ...
            js_set(
                &obj,
                "channel_id",
                &JsValue::from_str(&b64_encode(&channel_id)),
            );
            obj.into()
        }
        Err(e) => js_error(&e.to_string()),
...
pub fn byo_enrollment_derive_session(
    ...
    let ch_bytes = match b64_decode(&channel_id_b64) {
        Ok(b) => b,
...
pub fn byo_ed25519_verify(
    ...
    let obj = js_sys::Object::new();
    js_set(&obj, "valid", &JsValue::from_bool(ed25519_verify(&pk, &message, &sig)));
    obj.into()
}
...
pub fn byo_enrollment_derive_keys(session_id: u32, peer_pk_b64: String) -> JsValue {
    ...
    let result = with_enrollment_session_mut(session_id, |session| {
        ...
        let derived =
            enrollment_derive_session(&eph_sk, &peer_pk_bytes, &session.channel_id)
...
pub fn byo_enrollment_session_encrypt_shard(session_id: u32, shard_b64: String) -> JsValue {
    ...
    let result = with_enrollment_session_mut(session_id, |session| {
        let enc_key = match &session.enc_key {
            ...
            None => return Err("session enc_key not set (call byo_enrollment_derive_keys first)".to_string()),
        };
        let mac_key = match &session.mac_key {
            Some(k) => SymmetricKey::new(*k.as_bytes()),
            None => return Err("session mac_key not set".to_string()),
        };
        encrypt_shard_for_transfer(&shard, &enc_key, &mac_key)
            .map_err(|e| e.to_string())
    });
...
pub fn byo_enrollment_session_get_shard(session_id: u32) -> JsValue {
    ...
    match result {
        ...
        Some(Some(shard_bytes)) => {
            let obj = js_sys::Object::new();
            js_set(&obj, "shard_b64", &JsValue::from_str(&b64_encode(&shard_bytes)));
            obj.into()
        }
    }
}
...
pub fn byo_vault_open(
    ...
    let argon_output =
        match argon2id_derive_with_params(password.as_bytes(), &salt, memory_kb, iterations, parallelism) {
            Ok(o) => o,
            Err(e) => return js_error(&e.to_string()),
        };
    let vault_kek = match derive_vault_kek(&argon_output) {
...
pub fn byo_vault_derive_kek(session_id: u32, shard_b64: String) -> JsValue {
    let result = with_vault_session_mut(session_id, |sess| {
        let shard_bytes = b64_decode(&shard_b64)
            .map_err(|_| "invalid base64 shard".to_string())?;
        let kek = derive_byo_kek(&sess.client_kek_half, &shard_bytes)
            .map_err(|e| e.to_string())?;
        sess.kek = Some(kek);
        Ok::<_, String>(())
    });
...
/// Decrypt the ML-KEM and X25519 private keys using the session KEK.
///
/// Both `mlkem_sk_encrypted` and `x25519_sk_encrypted` are raw bytes in the
/// format `nonce(12) || AES-256-GCM(kek, nonce, plaintext)` — the same layout
/// stored in the `key_versions` table.
///
...
pub fn byo_vault_load_keys(
    ...
    let result = with_vault_session(session_id, |sess| {
        let kek = match sess.kek.as_ref() {
            Some(k) => k,
            None => return Err("KEK not derived — call byo_vault_derive_kek first".to_string()),
        };
        let mlkem_sk = vault_aes_decrypt(&mlkem_sk_encrypted, kek)?;
        let x25519_sk = vault_aes_decrypt(&x25519_sk_encrypted, kek)?;
    ...
    match result {
        ...
        Some(Ok((mlkem, x25519))) => {
            let obj = js_sys::Object::new();
            js_set(&obj, "mlkem_sk_b64", &JsValue::from_str(&b64_encode(&mlkem)));
            js_set(&obj, "x25519_sk_b64", &JsValue::from_str(&b64_encode(&x25519)));
            obj.into()
        }
    }
}
...
pub fn byo_vault_seal_device_signing_key(
    ...
    let result = with_vault_session(session_id, |sess| {
        let device_id_bytes = b64_decode(&device_id_b64)
            .map_err(|_| "invalid base64 device_id".to_string())?;
        let device_id: [u8; 16] = device_id_bytes
            .try_into()
            .map_err(|_| "device_id must be 16 bytes".to_string())?;
        let seed_bytes = b64_decode(&seed_b64)
            .map_err(|_| "invalid base64 seed".to_string())?;
        let seed: [u8; 32] = seed_bytes
            .try_into()
            .map_err(|_| "seed must be 32 bytes".to_string())?;
        seal_device_signing_key(&sess.vault_key, &device_id, &seed)
            .map_err(|e| e.to_string())
    });
    match result {
        None => js_error("vault session not found"),
...
pub fn byo_vault_unseal_device_signing_key(
    ...
    let result = with_vault_session(session_id, |sess| {
        ...
        let device_id: [u8; 16] = device_id_bytes
            .try_into()
            .map_err(|_| "device_id must be 16 bytes".to_string())?;
        let wrapped_bytes = b64_decode(&wrapped_b64)
            .map_err(|_| "invalid base64 wrapped".to_string())?;
        let wrapped: [u8; 48] = wrapped_bytes
            .try_into()
            .map_err(|_| "wrapped must be 48 bytes".to_string())?;
        unseal_device_signing_key(&sess.vault_key, &device_id, &wrapped)
            .map_err(|e| e.to_string())
    });
    match result {
        None => js_error("vault session not found"),
...
pub fn byo_vault_rewrap(session_id: u32, new_wrapping_key_b64: String) -> JsValue {
    let result = with_vault_session(session_id, |sess| {
        ...
        let wrapping_key = SymmetricKey::from_slice(&wk_bytes)
            .map_err(|e| e.to_string())?;
...
pub fn byo_vault_create(
    ...
    js_set(&obj, "master_salt_b64", &JsValue::from_str(&master_salt_b64));
    js_set(&obj, "pass_wrap_iv_b64", &JsValue::from_str(&pass_wrap_iv_b64));
    js_set(&obj, "pass_wrapped_key_b64", &JsValue::from_str(&pass_wrapped_key_b64));
    obj.into()
}
...
pub fn byo_vault_wrap_recovery(session_id: u32, recovery_key_b64: String) -> JsValue {
    ...
    match result {
        ...
        Some(Ok((nonce, wrapped))) => {
            let obj = js_sys::Object::new();
            js_set(&obj, "rec_wrap_iv_b64", &JsValue::from_str(&b64_encode(nonce.as_bytes())));
            js_set(&obj, "rec_wrapped_key_b64", &JsValue::from_str(&b64_encode(&wrapped)));
            obj.into()
        }
    }
}
...
pub fn byo_vault_rewrap_with_passphrase(
    ...
    let argon_output =
        match argon2id_derive_with_params(new_password.as_bytes(), &new_salt, memory_kb, iterations, parallelism) {
            Ok(o) => o,
            Err(e) => return js_error(&e.to_string()),
        };
    ...
    match result {
        ...
        Some(Ok((nonce, wrapped))) => {
            let obj = js_sys::Object::new();
            js_set(&obj, "wrap_iv_b64", &JsValue::from_str(&b64_encode(nonce.as_bytes())));
            js_set(&obj, "wrapped_key_b64", &JsValue::from_str(&b64_encode(&wrapped)));
            js_set(&obj, "master_salt_b64", &JsValue::from_str(&b64_encode(&new_salt)));
            obj.into()
        }
    }
}
...
#[wasm_bindgen]
pub fn byo_share_unwrap_key(salt_b64url: &str, encrypted_ck_b64url: &str, password: &str) -> JsValue {
