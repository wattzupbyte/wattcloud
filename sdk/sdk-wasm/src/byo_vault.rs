...
use sdk_core::byo::{
    decrypt_body, decrypt_manifest, encrypt_body, encrypt_manifest, merge_manifests,
    validate_manifest,
    cross_provider_move::{
...
pub fn byo_manifest_decrypt(session_id: u32, body_blob_b64: &str) -> JsValue {
    ...
    let result = with_vault_session(session_id, |sess| {
        decrypt_manifest(&sess.vault_key, &blob)
    });
...
pub fn byo_derive_per_vault_wal_key(session_id: u32, provider_id: &str) -> JsValue {
    ...
    match result {
        ...
        Some(Ok(key)) => {
            let obj = js_sys::Object::new();
            js_set(&obj, "key_b64", &JsValue::from_str(&b64_encode(key.as_bytes())));
            obj.into()
        }
    }
}
...
pub fn byo_cross_provider_move_decide_replay(
    ...
    match decide_replay(&step, dst_file_exists, src_blob_exists) {
        ...
        sdk_core::byo::ReplayDecision::Retry { provider_id, provider_ref } => {
...
pub fn byo_cross_provider_move_plan_reconcile(
    ...
    for action in actions {
        ...
        match action {
            ReconcileAction::DeleteBlob { provider_id, provider_ref } => {
...
pub fn byo_derive_manifest_aead_key(session_id: u32) -> JsValue {
    let result =
        with_vault_session(session_id, |sess| derive_manifest_aead_key(&sess.vault_key));
    match result {
        None => js_error("session not found"),
        Some(Err(e)) => js_error(&e.to_string()),
        Some(Ok(key)) => {
