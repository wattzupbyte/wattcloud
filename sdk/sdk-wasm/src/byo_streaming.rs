...
fn make_upload_session<P: StorageProvider + 'static>(
    ...
    UploadSession { flow: Some(flow), write, close, abort }
}
...
pub async fn byo_stream_upload_init(
    ...
    let cfg: ProviderConfig = serde_json::from_str(config_json)
    ...
    let (flow, header, total_size) =
        ByoUploadFlow::new(&mlkem_pub, &x25519_pub, plaintext_len as u64)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
    let opts = UploadOptions { parent_ref, ..Default::default() };

    macro_rules! init_provider {
        ($ty:ty) => {{
            let p = <$ty>::new(ReqwestProviderHttpClient::new());
            p.init(cfg).await.map_err(|e| JsValue::from_str(&e.to_string()))?;
            let stream_id = p.upload_stream_open(None, name, total_size, opts)
            ...
            p.upload_stream_write(stream_id.clone(), header)
                .await.map_err(|e| JsValue::from_str(&e.to_string()))?;
            let sid = session_id();
            UPLOADS.with(|m| m.borrow_mut().insert(sid.clone(), make_upload_session(flow, p, stream_id)));
    ...
    js_sys::Reflect::set(&obj, &JsValue::from_str("sessionId"), &JsValue::from_str(&session_id))
        .map_err(|e| JsValue::from_str(&format!("{e:?}")))?;
    js_sys::Reflect::set(&obj, &JsValue::from_str("chunkSize"), &JsValue::from_f64(V7_ENCRYPT_CHUNK_SIZE as f64))
        .map_err(|e| JsValue::from_str(&format!("{e:?}")))?;
    Ok(obj.into())
}
...
pub async fn byo_stream_upload_push(
    ...
    let (frame, write) = UPLOADS.with(|m| {
        ...
        let flow = s.flow.as_mut()
            .ok_or_else(|| JsValue::from_str("byoStreamUploadPush: already finalised"))?;
        let frame = flow.push_chunk(&plaintext, is_last)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        Ok::<_, JsValue>((frame, Rc::clone(&s.write)))
    })?;

    write(frame).await.map_err(|e| JsValue::from_str(&e.to_string()))
}
...
pub async fn byo_stream_upload_finalize(session_id: &str) -> Result<JsValue, JsValue> {
    ...
    write(footer).await.map_err(|e| JsValue::from_str(&e.to_string()))?;
    let result = close().await.map_err(|e| JsValue::from_str(&e.to_string()))?;
...
fn make_download_session<P: StorageProvider + 'static>(
    ...
    DownloadSession { flow, read, close, last_touch_ms: now_ms() }
}
...
pub async fn byo_stream_download_init(
    ...
    macro_rules! init_provider {
        ($ty:ty) => {{
            let p = <$ty>::new(ReqwestProviderHttpClient::new());
            p.init(cfg).await.map_err(|e| JsValue::from_str(&e.to_string()))?;
            let stream_id = p.download_stream_open(ref_)
                .await.map_err(|e| JsValue::from_str(&e.to_string()))?;
            let sid = session_id();
            DOWNLOADS.with(|m| m.borrow_mut().insert(sid.clone(), make_download_session(flow, p, stream_id)));
            Ok::<String, JsValue>(sid)
        }};
    }
...
pub async fn byo_stream_download_pull(session_id: &str) -> Result<JsValue, JsValue> {
    ...
    let read = DOWNLOADS.with(|m| {
        m.borrow().get(session_id)
            .map(|s| Rc::clone(&s.read))
            .ok_or_else(|| JsValue::from_str("byoStreamDownloadPull: session not found"))
    })?;

    let maybe_chunk = read().await.map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    match maybe_chunk {
        ...
        Some(raw_cipher) => {
            let plaintext = DOWNLOADS.with(|m| {
                let mut map = m.borrow_mut();
                let s = map.get_mut(session_id)
                    .ok_or_else(|| JsValue::from_str("byoStreamDownloadPull: session disappeared"))?;
                // S11: refresh the last-touch timestamp so the sweeper
                // doesn't evict an active session between chunks.
                s.last_touch_ms = now_ms();
                s.flow.push(&raw_cipher).map_err(|e| JsValue::from_str(&e.to_string()))
            })?;
...
pub async fn byo_stream_download_close(session_id: &str) -> Result<(), JsValue> {
    ...
    flow.finalize().map_err(|e| JsValue::from_str(&e.to_string()))?;
