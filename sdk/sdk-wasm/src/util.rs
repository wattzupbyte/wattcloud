...
///
/// Used for values that appear in URL query parameters or path components —
/// the enrollment channel_id is the canonical example. The relay's
/// purpose validator (`is_valid_purpose` in byo-relay) rejects standard
/// base64 padding, so producing padded output breaks `enroll:<ch>`.
pub fn b64url_encode_nopad(data: &[u8]) -> String {
    URL_SAFE_NO_PAD.encode(data)
}

/// Decode a URL-safe-no-pad base64 string; also accepts standard base64
/// with or without padding so existing callers that hand back the
/// original encoded form keep working.
pub fn b64url_decode_lenient(s: &str) -> Result<Vec<u8>, JsValue> {
